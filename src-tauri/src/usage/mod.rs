...
//!   - Data exists only for Claude.ai subscription auth, only after the first
//!     API response of a session, and only while a cImp-launched Claude tab
//!     runs with the statusline injection enabled. Absent/expired push data
//!     hides the widget — by design, there is no network fallback.
//!   - Either window may be independently absent.
...
/// `{"written_at_ms":…,"five_hour":{…},"seven_day":{…}}`.
...
fn interpret_push(raw: &str, now_ms: u64) -> UsageResult {
    ...
    // An empty snapshot is absence with extra steps — never render it.
    if pushed.snapshot.five_hour.is_none() && pushed.snapshot.seven_day.is_none() {
        return UsageResult::default();
    }
    ...
    if age > HIDE_AFTER {
        debug!(age_secs = age.as_secs(), "usage: push data expired; widget hides");
        return UsageResult::default();
    }
    UsageResult {
...
pub(crate) fn epoch_secs_to_iso(n: i64) -> Option<String> {
    let secs = if n.abs() > 100_000_000_000 { n / 1000 } else { n };
