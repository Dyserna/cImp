...
// for that field too).
let CIMP_GATE_STATE = {{ at: 0, gate: false, latch: "open", contaminated: false }};
...
async function cimpGateState() {{
  ...
  const open = {{ at: now, gate: false, latch: "open", contaminated: false }};
  ...
  try {{
    ...
    return settle({{ at: now, gate: true, latch: j.latch, contaminated: j.contaminated === true }});
  }} catch (_e) {{
    return settle(open);
  }}
}}
...
export default async (input) => ({{
  ...
  "tool.execute.before": async (inp) => {{
    ...
    try {{
      ...
      CIMP_GATE_EPOCH++;
      CIMP_GATE_STATE = {{ at: 0, gate: false, latch: "open", contaminated: false }};
...
const OPENCODE_PINNED_READ_ANY: &str = "allow";
const OPENCODE_PINNED_READ_ENV: &str = "ask";
const OPENCODE_PINNED_READ_ENV_EXAMPLE: &str = "allow";

/// [`OPENCODE_PINNED_READ_ANY`] & co. assembled in their load-bearing order.
fn opencode_pinned_read() -> serde_json::Value {
    serde_json::json!({
...
mod tests {
    ...
    fn the_native_gate_fails_open_on_every_error_path() {
        ...
        assert!(helper.contains(
            r#"const open = { at: now, gate: false, latch: "open", contaminated: false };"#
        ));
    ...
    fn the_native_gate_caches_state_and_drops_it_when_the_beacon_fires() {
        ...
        let drop = hook
            .find(r#"CIMP_GATE_STATE = { at: 0, gate: false, latch: "open", contaminated: false };"#)
            .expect("cache drop");
...
"#;
    ...
    ///      `webfetch` ADMITTED (fail open on a schema mismatch).
    ///
...
// …and the latch half still bites in the same reply shape.
