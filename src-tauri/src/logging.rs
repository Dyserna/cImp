...
//! workflow (`RUST_LOG=cctts=trace npm run tauri dev`) keeps working.
...
pub fn init(initial: LogLevel) -> WorkerGuard {
    ...
    let appender = rolling::daily(&dir, "cctts.log");
...
pub fn run_cleanup(retention: LogRetention) {
    ...
    for entry in entries.flatten() {
        ...
        // Match the appender's filename pattern: "cctts.log" prefix plus
        // a date suffix. Anything else in `logs/` is ignored.
        let name = match path.file_name().and_then(|n| n.to_str()) {
            Some(n) if n.starts_with("cctts.log") => n.to_string(),
            _ => continue,
        };
