...
| `claude.hook.tool_result` | Tool-result sizing, pushed (V35 Phase L) | A **second** `PostToolUse` entry with an all-tools matcher (`""`) of `type: "http"`, carrying `tool_name` and `tool_result` (string, or `{type:"text", text}` blocks). Deliberately a separate ROUTE from the auto-check entry: both matcher groups fire for an `Edit`, so one shared route would run the project's checks twice and count one result twice. Sized through the transcript reader's own `tool_result_chars`, so that reader's fixture canary is the leading check for both paths | `harness/claude/hook.rs` + `harness/claude/overlay.rs` → route `/claude/hook/post_tool_use_result` → `UsageEvent::ToolResult` | Tool-result sizes stop being recorded (the reader stays suppressed, on purpose). `drift.payload.v1` under `tool_result_hook`, either for a present-but-unsizeable `tool_result` or for the hook going quiet (witnessed by `context.post_edit` pushes) |
