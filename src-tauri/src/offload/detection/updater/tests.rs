...
async fn check_only_mode_records_the_offer_and_downloads_nothing() {
    ...
    let (available, failed) = signals_from(&tree.state());
...
/// A manifest that will not parse fails every enabled component loudly. A
/// silent no-op would be indistinguishable from "everything is current".
#[tokio::test]
async fn an_unparseable_manifest_fails_the_check_instead_of_looking_current() {
    let tree = Tree::new();
    tree.seed_rules("core.yar", GOOD_RULE);
    let mut map = HashMap::new();
    map.insert(
    ...
    let r = run_rules(&tree, Mode::Auto, &fetcher).await;
    assert_eq!(r.outcome, Outcome::Rejected, "{}", r.detail);
    assert!(r.detail.contains("update check failed"), "{}", r.detail);
    assert!(tree.live_rule_text("core.yar").contains("IgnorePrevious"));
    let (_, failed) = signals_from(&tree.state());
    assert_eq!(failed.len(), 1, "the failure reaches the Advisor");
}
