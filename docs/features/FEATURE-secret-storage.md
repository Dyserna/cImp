...
- `<launch_cwd>/.cctts.custom.config.json` (per-folder overlay)
...
For v1.x: revisit if anyone reports a leaked-token incident or if cctts grows a feature where the local-LLM tab routes through a paid third-party proxy that requires a real key (in which case Option A becomes table stakes).
