...
pub fn parse(
    ...
    match kind {
        ...
        ParserKind::TyposJsonl => parse_typos_jsonl(stdout, cwd),
        ParserKind::KnipJson => parse_knip_json(stdout, cwd),
        ParserKind::MacheteText => parse_machete_text(&strip_ansi(stdout), cwd),
...
// ── V38 Phase C ─ the three audit-era parsers, folded in ───────────────
//
// R2: one parser layer serves both umbrellas. These three arrived with the V25
// quality tools and lived in `audit::parsers` because nothing else consumed
// them; V38's plugin tools reach the SAME decoders through `ParserKind`, so
// keeping a second dispatch beside this one would be two places to add the next
// parser to. `audit::parsers::AuditParser` now delegates here (as its `Sarif`
// and `EslintJson` arms already did) and retires entirely in Phase E.
