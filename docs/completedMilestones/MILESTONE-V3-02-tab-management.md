...
- **Working directory**: `<input type="text">` with a "Browse..." button calling Tauri's `dialog.open` with `directory: true`. Default value: empty (interpreted as cimp's launch directory by the backend).
