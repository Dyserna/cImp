# Milestone V1.4-06: Per-Tab TTS Settings (Skeleton)

## Purpose

Item 4 from `FEATURE-per-tab-overrides.md`. Per-tab voice / speed / volume override so each tab can have its own TTS identity (e.g., a distinct voice for the Claude tab vs. aider tab). Read V1.4-01 first — V1.4-06 follows the same pattern, but the *resolver* lives in Rust because TTS happens in the backend.
