...
/// Are there any non-root empty panes in the tree? Used by the
/// integrity loop's collapse step to know when to stop iterating.
function hasEmptyNonRootPane(root: LayoutNode): boolean {
  if (root.type === 'pane') return false;
  for (const pane of eachPane(root)) {
    if (pane.tab_ids.length === 0) return true;
  }
  return false;
}
...
/// Adapt a persisted layout to the live tab list. Five concerns:
///
///   1. Drop tab ids no longer in `settings.tabs` — these are tabs the
///      user deleted between launches. The pane's `active_tab_id` is
///      reset to the first remaining tab if it was dropped.
...
export function validateAndRepairLayout(
  ...
  if (orphans.length > 0) {
    tree = walkPanes(tree, (pane) => {
      if (pane.id !== focused) return pane;
  ...
  while (tree.type === 'split' && hasEmptyNonRootPane(tree)) {
  ...
  // can only fire when `tabs` is itself empty, which the backend's
  // integrity check prevents — but a corrupt-mid-launch state could
  // still hit this and we'd rather rebuild than render a blank app.
