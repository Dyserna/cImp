//! **The `cimp --statusline` subcommand** — Claude Code's status-line
//! contract, end to end: the `statusLine.command` string cImp injects, the
//! child process that answers it, the coloured bar it prints, and the payload
//! parsing that turns the same stdin JSON into a usage push.
//!
//! V40 Phase D (locked decision 19) merged the two halves this used to be
//! split across. `crate::statusline` held the entry point, the shell quoting
//! and the renderer as though they were cImp surfaces; they are not — every
//! one of them exists because **Claude Code** runs an external command through
//! an unknown shell and reads one line back from its stdout. The flag reaches
//! `main.rs` through [`HarnessPlugin::subcommands`] now, so core does not know
//! `--statusline` exists.
//!
//! Two halves live here:
//!
//!   * [`launch_command`] builds the string handed to Claude Code's
//!     `statusLine.command` (via a `--settings` pre-arg in
//!     [`super::overlay`]). It points at *this very binary* re-invoked with
//!     `--statusline`, so there is no external script to ship and no
//!     dependency on node/python/PowerShell — the bar renders in Rust.
//!   * [`run`] is that re-invocation: Claude Code runs `cimp --statusline`,
//!     pipes the session JSON to our stdin, and reads the rendered bar from
//!     our stdout. `main()` dispatches to it through the plugin's subcommand
//!     table before any Tauri/audio/settings init, so it is instant and never
//!     spins up the GUI.
//!
//! Capability row: `claude.statusline.stdin` (Tier C, silent degradation —
//! canary-covered).
//!
...
//! in one payload via [`super::usage::store_pushed_usage`] for the bottom-bar
//! usage widget — that push is the widget's only data source (see
//! [`super::usage`] for the why and the file contract). Extraction happens
//! only after the bar has been written, and every field is optional, so
//! upstream shape drift can cost data but never the status line.
//!
...
/// The subcommand table [`super::plugin::ClaudePlugin`] declares.
pub const SUBCOMMANDS: &[crate::harness::plugin::Subcommand] =
    &[crate::harness::plugin::Subcommand { flag: "--statusline", run }];
...
pub fn run() {
    ...
    if let Some(snapshot) = extract_push(&input) {
        super::usage::store_pushed_usage(&snapshot, &extract_push_meta(&input));
    }
}
...
/// `pub(crate)` for the V35 canary suite (`harness/claude/canary.rs`).
...
// ---- payload parsing: see the `Input` types and extractors below --------
...
// ---- Claude-shaped payload parsing (V35 Phase K) -------------------------
...
pub(crate) fn extract_push(input: &str) -> Option<super::usage::UsageSnapshot> {
    let v: serde_json::Value = serde_json::from_str(input).ok()?;
    let (five_hour, seven_day) = extract_rate_limits(&v);
    let context = extract_context(&v);
    let snapshot = super::usage::UsageSnapshot {
        five_hour,
        seven_day,
        context,
    };
...
/// (see `super::usage::merge_push`), so it deliberately stays out of
...
pub(crate) fn extract_push_meta(input: &str) -> super::usage::PushMeta {
    let Ok(v) = serde_json::from_str::<serde_json::Value>(input) else {
        return super::usage::PushMeta::default();
    };
...
pub(crate) fn extract_rate_limits(
    ...
    Option<super::usage::UsageWindow>,
    Option<super::usage::UsageWindow>,
) {
    let Some(rl) = v.get("rate_limits") else {
        return (None, None);
    };
    let window = |key: &str| -> Option<super::usage::UsageWindow> {
        let w = rl.get(key)?;
        ...
        let resets_at = w.get("resets_at").and_then(|r| {
            r.as_str()
                .map(str::to_string)
                .or_else(|| r.as_i64().and_then(super::usage::epoch_secs_to_iso))
        });
        Some(super::usage::UsageWindow {
            utilization,
            resets_at,
        })
    };
...
pub(crate) fn extract_context(v: &serde_json::Value) -> Option<super::usage::ContextSnapshot> {
    ...
    let mut ctx = super::usage::ContextSnapshot {
