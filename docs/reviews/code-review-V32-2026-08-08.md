...
| F-21 | "The worker withholds defs, the proxy refuses the call" is wrong as a consumer-level rule — it is per **MODE** | **OPEN** | doc accuracy. Under a declared `profile` the other side's defs are genuinely absent; with `profile` omitted the worker keeps the full list and is refused at the gate. Containment intact either way. Same class as O-1 and F-26 |
