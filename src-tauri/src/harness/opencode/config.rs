...
/// The module's unit tests, in a sibling file (#132). Fifteen of them moved
/// here from `tabs::config`'s test module, where they had asserted on the
/// `OPENCODE_CONFIG_CONTENT` object this file writes while reaching it only
/// through [`build_opencode_config`]. The four that were already here are
/// unchanged; the shared fixtures are [`crate::harness::fixtures`].
#[cfg(test)]
