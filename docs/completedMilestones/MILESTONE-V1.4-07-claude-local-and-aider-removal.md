...
9. Aider TTS-injection deferred work in `FUTURE-FEATURES.md` (under "External dependencies → Aider TTS markup injection") moves to the historical section. cimp no longer has an aider tab to inject into; the upstream-aider blocker is irrelevant to cimp going forward.
...
15. **Settings UI: AI section.** A new "AI" section in the Settings window (or a subsection of the existing Tabs section) exposes:
    ...
    - Help text naming the supported backends (LM Studio, Ollama, vLLM, llama-server) and noting that the backend must be running separately (cimp does not auto-spawn it).
...
- **Auto-spawn the local backend.** cimp does not bundle, install, or start LM Studio, Ollama, llama-server, vLLM, or any external translator. The user runs the backend themselves (a Shell tab is a natural place to start it). Documented in README. A future "backend sidecar" feature is a candidate for FUTURE-FEATURES.md if real-use friction surfaces.
- **Bundle a local model** or any provider config beyond the env vars. cimp is provider-agnostic past the env-var injection. The user picks their own model in their backend's config (LM Studio, Ollama, llama-server, vLLM).
...
- **Tool-use compatibility certification.** Local models vary in tool-call reliability. cimp displays whatever the model emits; broken tool use is a model issue, not a cimp bug. Documented in README under "Local LLM caveats."
...
<section class="ai-section">
  ...
  <p class="hint">
    ...
    Ollama, or vLLM. cimp does not start the backend — launch it separately.
...
- **No backend-up indicator.** If the user toggles `use_local_provider` but their backend isn't running, Claude Code launches and fails on first message with a connection error. cimp shows the error in the avatar/status bar via the existing error path, but doesn't proactively check the backend. Out of scope; if real-use friction surfaces, add a "ping the backend on tab spawn" warning.
...
### Phase D
- **The aider id rewrite is irreversible without the backup.** A user who lost their `config.json.v1.7.bak.<ts>` and wanted aider back could rebuild manually but their layout-tree positioning of "aider" is now `claude-local`. Backup path documented; this is the standard cimp migration model, no novel risk.
