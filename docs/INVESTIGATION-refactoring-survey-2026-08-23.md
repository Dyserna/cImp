...
| R7 | **advisor.rs split** — `drift_rules` is one 744-line fn (`advisor.rs:975-1719`) whose detection/updater advisor block (329 lines, reading only `offload::detection` types) sits **before** the final `drift.read_bypass.v1` card rather than at the tail (corrected 2026-08-24 from the #118 split); its 6 `Signals` fields + 580 test lines partition cleanly. `advisor/{drift,detection}.rs`. Preserve card order; keep `ALL_RULE_IDS` reachable. | M | B |
