...
Migrate the v1.3 drag-and-drop implementation from `mouse*` events to `pointer*` events, so a single code path supports mouse, touch, and pen input. Enables tab dragging and splitter resize on touchscreen laptops, tablets running cImp, and accessibility devices that emulate pointer events.
...
**Trigger to act**: per `FUTURE-FEATURES.md`, "if you ever run cImp on a touchscreen device, or accessibility need." Don't pre-emptively pick this up — touch isn't part of the primary user's daily driver setup, and the current mouse-only implementation works fine for the primary use case.
