...
- [ ] **Next:** TS/JS/Python symbol extraction (rest of Phase E — needs the tree-sitter grammar crates, version-matched to ts 0.26); ast-grep `struct_search` (Stage 4); loopback `POST /graph/query` warm path for the cloud MCP child (avoids the read-only double-open during a rebuild); rest of settings UI + `graph_pause_watch`/`graph_rebuild_embeddings`/`graph_test_query` (F); semantic/embeddings (G); monitor tab (I).
