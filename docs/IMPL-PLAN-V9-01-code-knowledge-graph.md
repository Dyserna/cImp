...
- [x] **Phase G (semantic search)** — embed client + CozoDB HNSW vector store + epoch + opportunistic backfill + `graph_semantic_docs` (full-text fallback) + status. Committed `0ae7740`.
- [x] **Phase I (monitor tab)** — reserved app-rendered Code Graph tab + `GraphMonitorView`. Committed `469e969`.
- [x] **Phase F (settings UI)** — full graph config panel. Committed `f90f04e`.
