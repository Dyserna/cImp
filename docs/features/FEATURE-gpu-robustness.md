...
The underlying ORT 1.20 + Blackwell mismatch is also tracked in `docs/MAINTENANCE.md` under "ort / ONNX Runtime." This feature complements that maintenance entry — the maintenance entry tracks the upstream upgrade; this feature improves cctts's behavior *until* the upstream upgrade lands and *after* it lands for any next-gen-GPU regression class.
...
Find where `ort` is initialized today and where the CUDA EP is registered. Likely in `src-tauri/src/tts/...` (a `tts/init.rs` or similar). Confirm the `CCTTS_GPU=cuda` env var is read there.

### 2. Add the CC probe

Before EP registration, when `CCTTS_GPU=cuda`:
...
```rust
if env::var("CCTTS_GPU").as_deref() == Ok("cuda") {
...
The warning message is the user-visible payoff. Include the CC numbers and a doc reference so users know what to do (upgrade cctts when an ort bump lands; until then, accept CPU).
...
- **AMD ROCm / Intel oneAPI / Apple MPS**: out of scope. cctts targets Windows and Linux with NVIDIA, per `DESIGN.md`.
- **What if `cudarc` itself fails to load** (e.g., no NVIDIA driver, no CUDA runtime)? Treat as "probe failed → fall back to CPU." User probably set `CCTTS_GPU=cuda` accidentally on a machine without CUDA. Don't crash; warn and continue.
