...
- Claude Code permission detection, hook-primary: unconditional `Notification` + `PermissionDenied` hooks (`cimp --notify-hook` → `POST /permission/event`) flip the awaiting-permission badge/announcement, with the TUI footer matcher kept as the fallback for anything the hook drops (unmappable session, older CLI)
