//! The graph tool surface itself: the `graph_*` / `context_*` specs, the MCP
//! adapter both consumers reach it through, and the dispatch+format core.
//!
//! Two consumers share this file:
//! - the **cloud Opus session**, via the `cimp --offload-mcp` server (the
//!   [`tools`] descriptors + [`handle_call`]); and
...
use crate::graph::index::{ArchReport, DocHit, GraphIndex, PathHit, RefHit, SymbolHit};
use crate::graph::memory::{MemNote, ProjectFact, WorkingSetEntry};
...
fn record_secret_screen_flag(
    ...
    let detail = crate::graph::secrets::write_notice(hits);
    record_flag(Flag {
...
pub fn run_tool(
    ...
    match name {
        ...
        "context_note" => {
            ...
            let note = crate::graph::secrets::ScreenedNote::of(crate::graph::secrets::NoteText::parse(
                req("text")?,
            )?);
            ...
            idx.mem_add_note(&note_id, &sid, &note, ts, pin, guards.taint)
                .map(|_| {
                    ...
                    if !secrets.is_empty() {
                        out.push_str(&crate::graph::secrets::write_notice(secrets));
                    }
                    out
                })
                .map_err(|e| e.to_string())
...
async fn semantic_query(
    ...
    let Some(mut embedder) = crate::graph::embed::Embedder::new(
...
fn conf_badge(c: Option<crate::graph::model::Confidence>) -> String {
...
fn fmt_recent_changes(rows: &[crate::graph::gitmeta::FileChurn], max_rows: usize) -> String {
    if rows.is_empty() {
    ...
    let mut lines: Vec<String> = rows
        ...
        .map(|c| {
            format!(
                ...
                c.file,
                c.touches_90d,
                crate::graph::gitmeta::truncate_subject(&c.last_subject, 60),
                crate::graph::gitmeta::relative_age(now, c.last_ts)
            )
        })
        .collect();
...
fn run_struct_search(
    ...
    let lang = crate::graph::model::Lang::from_tag(lang_tag);
    if crate::graph::builder::language_for(lang).is_none() {
        return Err(format!(
    ...
    let hits = crate::graph::builder::struct_search(lang, pattern, &files, max_rows, max_snippet)?;
    if hits.is_empty() {
...
fn run_snippet(
    ...
    let stale = matches!(
        ...
        Some(stored) if stored != crate::graph::model::fnv1a_hex(&content)
    );
...
fn run_repo_map(
    ...
    let map = crate::graph::context::repo_map(idx, budget, &boost);
    if map.is_empty() {
...
/// ([`crate::graph::impact::changed_symbols`]) — then find their transitive
...
fn run_impact(
    ...
    let (root_names, changed_syms, unindexed): (Vec<String>, Vec<SymbolHit>, Vec<String>) =
        if !symbols_arg.is_empty() {
            ...
            let set = crate::graph::impact::changed_symbols(root, idx).map_err(|e| e.to_string())?;
    ...
    let min_confidence = match args.get("min_confidence").and_then(|v| v.as_str()) {
        Some(s) => Some(crate::graph::model::Confidence::parse_tag(s).ok_or_else(|| {
...
fn fmt_path(from: &str, to: &str, hit: &PathHit) -> String {
    ...
    let loc = |n: &crate::graph::index::PathNode| {
        if n.kind == "file" {
            format!("{} [file]", n.file)
        } else {
...
fn repo_map_session_boost(
    ...
    ws.iter()
        .map(|e| (e.path.clone(), crate::graph::context::session_weight(&e.last_kind)))
        .collect()
}
...
mod h1_signature_strip_tests {
    ...
    fn the_non_model_facing_consumers_still_get_the_signature() {
        ...
        let advice = crate::graph::context::read_advice(&idx, &dir, "src/pay.rs", None, false, 0);
        assert!(
