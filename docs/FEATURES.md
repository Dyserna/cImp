...
- A **failed check with zero diagnostics says why** (V38 F-2): when the exit code is non-zero and the parser produced no groups — the one case the structured report is mute — `run_check` appends up to 1 KB of the run's own last output lines under `raw output tail (unparsed):` (stderr's tail preferred). This is what turned a live `exit 101 · 140 ms — No diagnostics.` into the line that explained it. A clean run and a failure whose diagnostics parsed carry nothing extra
...
- **Detect** button (`audit_detect_tool` IPC) resolves the tool honoring the override and probes `<tool> --version`, shown inline (`✓ <version> — <path>` / not found). With a NON-empty path it is display-only. With an EMPTY one it searches ebin → PATH for the name the manifest implies (a built-in's `command`, a check's program, a command tool's id — bare names only) and, on a hit, selects it: the found path is stored as the machine-wide path for that tool and for every path-less sibling of the same plugin that resolves to the same binary (one Detect on `cargo build` also fills `cargo test`, `cargo clippy`, `cargo`). The IPC itself still writes no settings — the pane does, on the click. Run-time trust is unchanged: a user plugin's tool with no stored path still never runs
...
- **Command tools reach the AI tabs (V38 F-3).** A plugin `command`-kind entry that is enabled and has a binary path is callable from Claude Code / OpenCode tabs through one MCP tool, `run_command`, whose `tool` parameter is an enum of this project's runnable entries (the same short-id-else-qualified-key naming `run_check` uses). The registered binary is spawned **directly with the model's `args` as an argv vector — no shell** — in the project root; the cwd is not model-controllable, the per-program `CommandPolicy` argument rules still apply, and the manifest's sandbox posture reaches the spawn exactly as it does for the offload worker (a tool that declares `sandbox: required`, which is the default, does not run with OS sandboxing off). Two switches in Settings → Tool Plugins ("Command tools in AI tabs"), both **on** by default, gate it per consumer; the tool is hidden entirely while no command tool is runnable, and the listing rides the same surface pulse as `run_check`'s enum, so configuring a path updates a live Claude session without an app restart (OpenCode caches `tools/list` at connect — restart that tab). The offload worker's own `run_command` is unaffected by these switches
