# Baseline full-codebase review — 2026-06-28 (develop @ 1d61797)
...
**Direction:** bounded-but-larger or per-tab channels, or treat these control signals as must-deliver (send().await off the blocking path / a coalescing latest-wins cell). Same family as the window-resize false-notification already fixed at 76ca105.
...
- Status-bar regroup (97e01ff), AvatarToggleButton delete (dbc8f47), broot `-h` (d615063), no-models packaging (1d61797): no correctness bug found.
- Resize false-notification fix (76ca105): no new bug; one cleanup/low at pty/manager.rs:301 — poisoned-mutex if-let-Ok ignored but `Resized` still sent (add warn!). The rx/tick duplication at pty/tasks.rs:269 predates it.
