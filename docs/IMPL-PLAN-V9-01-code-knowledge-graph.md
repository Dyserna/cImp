...
- [ ] **Next:** ast-grep `struct_search` (Stage 4); loopback `POST /graph/query` warm path for the cloud MCP child (avoids the read-only double-open during a rebuild); rest of settings UI + `graph_pause_watch`/`graph_rebuild_embeddings`/`graph_test_query` (F); semantic/embeddings (G); monitor tab (I). (stack-graphs precise resolution remains an optional later upgrade over the current name-level edges.)
