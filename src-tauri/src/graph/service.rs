...
/// V42 R6 (#117): the read advisor — the verdict, the snapshot LRU, the
/// first-read tier and the bypass tap, plus the three service methods that
/// drive them. A child module, so it reaches this file's private state
/// (`reminded`, `read_seen`, `read_seen_touch`, `bypassed_advice_chars`)
/// without any of it having to widen.
mod readadvisor;

use readadvisor::{ReadSeenStore, RemindMark};

/// V42 R6 (#117): the live-session registry — which agent sessions are
/// running right now, which tab each is bound to, and whether that binding is
/// provable. Same shape as `readadvisor`: a child module, so the two maps it
/// decides over (`live_sessions`, `live_tab_roots`) stay private fields here.
mod live;

use live::{compute_active_session_ids, LiveRegistry, LiveTabRoot};

/// V42 R6 (#117): the free-function floor — the tree walk, the language
/// census, the embed plumbing and the warm-index open. Not a state bucket:
/// ~790 lines with no `&self` anywhere, which is why it was already testable
/// against a bare `GraphIndex`.
mod walk;

pub use walk::{LangCensus, RebuildOrigin};
use walk::{
    analyses_changed, build_gitignore, build_tree, embed_batch_isolated, embedding_epoch,
    index_dir_tree, index_push_worthy, lang_for, language_census, rel_path, relativize_path,
    resync_tree, warm_index, DirWalk,
};
...
pub struct GraphService {
    ...
    /// V17 Phase A: the value is now a [`readadvisor::ReadSeen`] carrying an optional
    /// in-memory snapshot of the last-read content, so a changed re-read can be
    /// answered with a diff. LRU-bounded (snapshots by [`readadvisor::SNAP_TOTAL_MAX`] bytes,
    /// rows by [`readadvisor::READ_SEEN_MAX_ENTRIES`]); eviction drops content, never the
    ...
    /// [`readadvisor::SNAP_TOTAL_MAX`] budget without re-summing the whole map each time.
    ...
    /// session id on the OpenCode loopback path), value = [`live::LiveSession`].
    ...
    /// also expires by [`live::LIVE_SESSION_TTL_MS`].
    live_sessions: StdMutex<LiveRegistry>,
    /// H1 fix: the running-tab → transcript-root map behind
    /// [`live::tab_binding_is_ambiguous`] — see [`live::LiveTabRoot`]. Written only by the
