...
- [ ] **Next:** ast-grep `struct_search` (Stage 4); loopback `POST /graph/query` warm path + native-tool router entry for the offload worker (rest of Phase C); watcher (D — `notify`, incremental re-index on change, staleness check on open); more langs/docs (E); settings UI + `graph_pause_watch`/`graph_rebuild_embeddings`/`graph_test_query` (F); semantic/embeddings (G); monitor tab (I).
