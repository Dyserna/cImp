...
Allow a tab (or a pane's worth of tabs) to be torn out of the main window into a new top-level cImp window. Standard browser-style tab-tearing pattern. The headline use case is multi-monitor: arrange Claude on monitor 1, aider + shells on monitor 2, with each window holding its own multi-pane layout tree.
...
- **Tauri 2.x WebviewWindow ergonomics**: Tauri 2.x is what cImp is on today. Verify at implementation time that `WebviewWindow::new` works the way assumed above; test that webview-to-webview communication via the broadcaster pattern actually works (it should — events go via the Tauri core).
