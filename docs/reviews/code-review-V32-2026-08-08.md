...
**Range:** `f4b0a03~1..74e297e` (V32 phases A–H + both fix runs) — 79 non-doc files,
34,400 insertions / 2,172 deletions.
**Reviewed at:** `74e297e` (develop), 2026-08-08.
...
|---|---|---|---|
| H-1 | C-1 open — `graph_struct_search`/`graph_repo_map` are TRUSTED source readers | **FIXED** | `7a38a05` — demote both + strip `signature`; amendment 2026-08-08 (c) + decision 29 |
| H-2 | C-2 open — contamination clears on any newline-terminated byte | **FIXED** | `9db7f10` — reset removed, not re-armed; amends decision 15 |
| H-3 | SSRF — `http:/host`, `http:host`, `http:\host` evade the screen | **FIXED** | `3a0e31e` — scheme parsed not matched; 15,840-case generated corpus |
| H-4 | Every shipped rule defeated by invisible whitespace | **FIXED** | `c1722f7` — rules + normalizer + corpus; 20/50 evading → 0/53 |
| H-5 | Update channel inert by construction (302 vs redirect ban) | **FIXED** | `c1722f7` — `raw.githubusercontent.com` branch + the missing guard |
...
| H-8 | `/audit/run`'s taint gate is opt-in by the caller | **FIXED** | `11b4dba` — `tab` required + `consumer` narrowed; both halves |
| H-9 | Forensic activity rows are model-evictable | **FIXED** | `0f38d51` — one retention lane per `Screen` (not the proposed pin); lane set derived from `Screen::ALL` |
| H-10 | A failed detection-status read renders the layer ARMED | **FIXED** | `db3e6db` — `SignatureHealth` has no `null`; unknown travels as a hierarchy row |
| F-2 | `^` anchors to byte 0, not line start — three patterns dead | **FIXED** | `c1722f7` — folded into H-4 (same file, same class) |
| M-4 | A classifier that ran and failed is indistinguishable from clean | **FIXED** | `c1722f7` — `Scored.failed` → `incomplete`, scored windows kept |
| N-8 | Unbounded `previous/classifier/<version>` retention | **FIXED** | `c1722f7` — died with the classifier component |
| N-10 | Corpus-echo bundle passes the whole gauntlet | **PARTIAL** | `c1722f7` — `coverage_floor` catches the curation case, not a hostile publisher |
| N-12 | 512 MiB aggregate-download exposure | **FIXED** | `c1722f7` — died with the classifier component |
...
| — | Pre-existing 1-in-4 test flake on the global rule slot | **FIXED** | `c1722f7` — guard made structural in `screen_blocking` |
| F-3 | The primary contamination path writes no forensic record at all | **FIXED** | `b81703e` — `Screen::Contamination` on the transition, both paths, one helper |
...
| F-9 | The signature scan's 1 s budget is wall clock spanning both passes | **FIXED** `1fa3be3` — **and it closed a CI FLAKE** | `SCAN_PASS_TIMEOUT = 2 s` **per pass** (the smallest value with a non-zero floor — nothing between 1 and 2 exists after `ceil()`); `SCAN_PASS_GUARANTEED = 1 s` is the only figure a consumer can honestly use; `SCAN_TIMEOUT` **deleted** so its consumer became a compile error, plus two `const assert!` tripwires. F-9b re-verified **in the dependency source**, not taken from the plan. **⚠ THE GAUNTLET RE-PRICING WAS DECIDED AGAINST THE PLAN, and the argument is the useful part:** the plan proposed 2 s (the sum of both floors); that is only right when work splits **evenly** — a document needing 1.8 s raw + 0.2 s normalized totals 2 s and passes while its raw pass blows its own floor, and a document that normalizes to itself costs **one** pass, making 2 s loose by 2×. A measured *total* bounds *either pass alone*, so `SCAN_BUDGET = SCAN_PASS_GUARANTEED = 1 s` is conservative in every split **and keeps the gauntlet's ceiling exactly where it was — F-9 loosens it by nothing.** Stated cost: a document whose two passes each take 0.9 s would complete in production and is rejected here; fails **closed**, and visibly. **⚠ THE REAL PAYOFF: `graph::secrets::tests::each_rule_matches_its_own_sample` IS NOW STABLE** — green across a normal run and two full runs at `--test-threads=32`, the oversubscription that reproduced it, with no timeout note firing anywhere. **That flake was F-9b's zero floor firing under load, so this closed a flake that had been reddening release commits.** Worst case 1 s → 4 s as accepted; no caller bound needed changing (nearest 30 s / 45 s, re-verified). Rationale + the two rejected options in the F-9 entry below. **SIZING CORRECTED 2026-08-11 — see F-9b: it must be 2 s PER PASS (`SCAN_PASS_TIMEOUT`), not 2 s total, or the fix is a no-op.** Floor becomes 1 s ≈ 4.8× the worst legal input (256 KiB ⇒ ~210 ms/pass, derived from the measured 105 ms per 64 KiB across both passes). **Worst-case wall clock 1 s → 4 s**; no caller bound needs changing (nearest are 30 s and 45 s). `SCAN_TIMEOUT` is to be **deleted, not aliased**, so its one consumer (`validate.rs:70`) becomes a compile error — a tripwire, per session 4's lesson. `DidNotComplete` must also name the pass (`raw`/`normalized`): today both passes emit identical strings and `merged_with`'s dedupe hides which layer died |
| F-9a | `Hits ⊕ DidNotComplete = Hits` — merging outcomes **drops the incompleteness**, so a truncated scan that happened to find something reports as a *complete* scan | **FIXED** `1fa3be3` | `ScanOutcome::Hits` is now a struct variant carrying `incomplete: Option<String>`; `merged_with` **merges** reasons instead of dropping them; the flagged-and-incomplete arm sets a gap. `DidNotComplete` now **names the pass** (`raw`/`normalized`) — previously both passes emitted identical strings and the dedupe hid which layer died. The assertion that had pinned this as the contract is inverted. **Bounded residual, recorded not hidden:** the hits-only `scan_with` still cannot distinguish timeout-with-no-hits from clean, because its two callers (`graph::secrets`, the gauntlet) take `Vec<String>`. F-9 narrows it (floor 0 → 1 s) and `scan_once`'s `warn!` still gives the fact a consumer; only the *return value* lacks it. Closing it means migrating callers, which decision 22 records as deliberate for `secrets` | raised 2026-08-11 while designing F-9's fix, recorded rather than folded in as silent scope creep. Same family as M-5: the honest signal exists and is then discarded. Note the related fail-open is *narrowed* by F-9's fix but not closed — truncation cannot be mistaken for clean through `scan_outcome_with`, but it **can** through the hits-only `scan_with` used by `graph::secrets` and the updater gauntlet |
...
| F-18 | Every pointer to the V32 controls names a Settings section that does not exist | **FIXED — ALL HALVES** `823acd9` (frontend + docs) + `aadf49d` (Rust + tripwire) | ⚠ **THE POPULATION WAS FAR LARGER THAN THE FINDING OR MY BRIEF PREDICTED: 36 pointers across 18 files, 27 of them wrong, and FIFTEEN user-visible** (refusals, advisor rationales, probe messages, tool descriptions) — not the "~20, about five user-visible" I estimated. **And a 28th was INVISIBLE TO GREP: `advisor.rs` had the ARROW ITSELF split across a `\` continuation**, so no search for `Settings →` could ever have found it, and it is a user-visible advisor rationale. **That single case is the whole argument for a tripwire over a sweep.** The tripwire parses labels out of `SettingsApp.svelte`'s own `SECTIONS` array (no hand-copied list), joins `\`-continued literals before matching, scans string literals **and doc comments**, and asserts it saw >100 files and >0 pointers so an empty scan fails. Its control reconstructs `backend_gate.rs`'s wrapped refusal byte-for-byte and asserts a per-line scan sees only `\` — **the vacuous pass, demonstrated rather than described.** It fired for real on the first full run (*"3 of 38 settings-path pointers name no sidebar section"*), which is also how the grep-invisible defect surfaced. Residual: only the **first** segment is validated, so a pointer can still name the right section and a wrong sub-heading — stated in the test's own doc. | Per the user decision the controls are now their own top-level category labelled exactly **`Injection protection`**. Verified as a pure move (the relocated 577-line block is byte-identical to `HEAD` apart from three intended edits). They were actually **one click deeper** than the finding described — inside Offload task tools → **Pool**. The ⛨ badge now deep-links to the section (it previously called `openSettingsWindow()` with no argument and landed on the default section). **Deliberately NOT aliased `offload → injection`:** the `RETIRED_TAB_IDS` precedent exists because *tab* ids are persisted, whereas `offload` is still live and still means the pool — an alias would hijack every legitimate link to it; a breadcrumb was left instead. **Native web is now honest about `sensor`** — one normalizer mirroring Rust's `parse` (so unrecognized resolves to `sensor`, not `off`, closing a TS/Rust divergence on `" off "`), the mode spelled out in force, and an empty-is-not-absent hole closed (a `<select>` whose stored value matches no `<option>` rendered **blank**, reading as "unset" while the backend enforced `sensor`). **New tripwire parses the sidebar labels out of `SECTIONS` itself** (no second copy to drift) with a positive control proving it catches the F-18 string. Because it enforces *all* pointers, every wrong first segment in `src/` was fixed — sweeping up **six more user-visible strings and eight wrong comments**, plus a `MAINTENANCE.md` pointer at a nonexistent "Audio" section. **⚠ RUST HALF OWED: `ipc/commands.rs`'s `updates_allowed` refusal (the fifth site), `backend_gate.rs`'s refusal that OUR OWN F-12 fix shipped the same day naming "Code Intelligence → Checks" when Checks is a top-level SIBLING, plus ~20 sibling pointers of which five are user-visible. THE TRAP FOR THAT TRIPWIRE: `backend_gate.rs`'s refusal wraps across lines with a `\` continuation, so a line-by-line scanner sees `Settings → ` with an empty tail and skips exactly the string F-12 just shipped — it must join continued literals first.** Residual reported, not fixed: Rust's `effective(Feature::NativeWeb)` is `mode != Off`, so the chip and the "reduced" list treat `sensor` as fully on and never say report-only. found live 2026-08-10 on v0.51.0-rc.1 — the first defect found by *running* the build |
| F-19 | No `claude-opus-5` row in the seeded price table — session cost reads $0 | **FIXED** | `8c09328` — seed row **plus** a `pricing_seeded_generation` watermark migration for existing installs; tripwire `every_built_in_priced_model_is_reachable_by_existing_installs` |
| F-10 | `NativeRouter` never re-gated `graph_*`/audit tools — `LOCAL_DATA_TOOLS` lists only read_file/list_dir/code_search/run_command/filesystem/git | **FIXED** | `c337d6a` — carried by M-2's `GatePass`; exploitable half was `graph_*` on a cloud/LAN backend via the headless child |
| F-11 | `select_discovery` prefers the DEEPEST matching root — one `Write` of a well-formed discovery file with a dead port forces the fallback *and* chooses the reason it reports | **FIXED** `7673d39` — locked decision 30 | Candidates are probed with a **token-authenticated `GET /health`** and skipped if they do not answer; memoized per process (successes only); re-resolved on failure by all four consumers. `HttpStatus`/`Unparseable` **stop falling back** — the app answered and *declined*, and re-running headless hid its verdict. **Bounds:** 200 ms **total** per candidate (connect/write/read share one deadline, so a candidate cannot cost 3×), ≤6 probes per **resolution** not per step, worst case **1.2 s once per process per hint**; a dead loopback port refuses instantly, so the 200 ms is only spent against something *listening and not answering* — the attacker who already paid for a listener. Budget exhaustion reads as a **non-answer**, never a free pass. **All four shims stay FAIL-OPEN — no shim gained a refusal path.** ⚠ **THE SEAM BUG THE TWO HALVES CREATE TOGETHER, caught and closed: the memo must be invalidated on EVERY miss, not just `Transport`.** A rotated token surfaces as `HttpStatus(401)`, and with fallback removed an un-invalidated memo would **wedge that tab permanently** — two individually correct changes producing a failure neither had. **Accepted residuals:** `sweep_stale_discoveries` still probes with a bare TCP connect, so a planted entry backed by *any* listener survives the app-start sweep (switching it to the real probe would delete a live sibling's entry on one flaky probe, and entries are only rewritten at instance start ⇒ permanent undiscoverability — declined deliberately); and >6 planted deeper entries push the real instance past the budget ⇒ headless, which is not a regression since today *one* entry does that. steers `audit/mcp.rs` and every hook shim; decide with F-26, which shows the same file set is larger than documented |
| F-12 | `run_check` is **advertised** to a cloud/LAN backend and executes the project's configured commands | **FIXED — ALL HALVES** `1306216` (backend) + `2af9fe8` (mirror + opt-in control) + `aadf49d` (mirror tripwire) | Both halves shipped: `run_check` added to `LOCAL_DATA_TOOLS` (`schema.rs:2916-2951`) **and** `BackendGate` rule 4 enforced **at call time** via a new `allow_run_check` resolved in `for_worker` from the new root setting `checks_allow_remote_worker` (default **false** = denied). All three worker entry points funnel through `for_worker` (`service.rs:1461`, `offload/mcp.rs:2040` headless child, `supervisor.rs:572` self-test), so an **already-configured** backend is fixed on its next call with no re-save. `BackendGate::new` gained a 4th **required positional** arg as a compile-error tripwire, so a future rule cannot be added with a permissive default. Verified: 23 `run_check` tests, full suite 1944 pass / 4 ignored, clippy clean; the opt-in round-trips through the sparse project overlay and **dropping the key returns it to the global denied value**. ⚠ **A SCHEMA BUMP WAS UNAVOIDABLE (29 → 30):** the pre-existing tripwire `local_data_tools_growth_requires_a_backfilling_migration` *demands* a backfilling migration for any addition to the set — this brief had framed the migration as unnecessary and was wrong. The v21→v22 literal was deliberately NOT widened (a user at v29 never re-runs that step, so it would have turned the tripwire green and fixed nobody's install). **Residual: a gate refusal writes NO `activity` row** — pre-existing for this whole class, same gap F-20 tracks, neither widened nor narrowed. HIGH. **User decision: add `run_check` to `LOCAL_DATA_TOOLS`, denying it to cloud backends BY DEFAULT, with a user opt-in.** Both halves are required and neither is sufficient alone: `LOCAL_DATA_TOOLS` fixes **new** backends, and the `BackendGate` opt-in must be enforced **at call time** so **existing** configured backends are fixed too (this is exactly F-10's shape — *"the helper is right, the call site is missing"*). Implement with M-8's residual and M-7 residual #1, one family |
| F-28 | One `Write` of a discovery file **silently disarms the native-web taint beacon** — `taint_beacon.rs:182` uses the same steered `read_discovery_for` as F-11, and the shim is fail-open by design | **FIXED** `7673d39` — closed by F-11's selection change | Proven against a **real socket** that 200s only on the matching bearer token, with a real dead port, using the real probe rather than a stub: `a_planted_dead_entry_no_longer_disarms_the_native_web_beacon`. ⚠ **Residual, flagged rather than silently accepted: those tests pin the SELECTOR the beacon calls, not the one line in `dispatch` that calls it.** No compile-error or non-source-scan way to pin that link was found, **so F-28's named live-verification row still matters — the beacon's POSITIVE leg is what proves the fix.** This is a sharper consequence than anything in F-11's own filing: F-11 steers *which instance answers and what error story is told*; F-28 turns off **locked decision 14's signal** outright. Fail-open is correct for a shim and is not the defect — the defect is that the resolution it fails open *from* is attacker-steerable. **Fix it in the F-11 pass** (liveness-verified selection), but keep the row so the beacon-disarm case is re-tested by name rather than assumed covered |
| F-29 | `outbound::scope_attribution` (`outbound.rs:1685`) maps a **colon-less scope to `Headless`**, so `record_secret_screen_flag` and `unattributed_write` rows falsely claim *"positively no tab"* | **FIXED** `aadf49d` | `Flag` now carries an explicit **non-`Default`** `attribution` field, so 15 production call sites plus every test literal had to state it — the compiler enforces it, no scan. `record_secret_screen_flag` threads the **real tab**, taken from `dispatch_recorded`'s already-classified value, so the secret-screen row and the call's own `kind:"graph"` row **cannot name different tabs** (25 `run_tool` sites updated). `unattributed_write` is deliberately `Unattributed` rather than more precise: `gate` receives `Option<&LatchScope>`, so `LatchScoping`'s Anonymous/Unknown split is already collapsed before that frame — recovering it means threading `CallProvenance`, stated in a comment rather than guessed. New `LatchScope::attribution()` reads the `tab` field instead of re-splitting the label, and `LatchScoping::attribution` delegates to it **so the two cannot drift**. Proven, not just plumbed: a test drives the real `context_note` arm with `Tab("claude-1")` and asserts the row's tab, so a regression to `Headless` fails. F-20's shape, different producer — `Headless` is a positive claim ("a worker run with no tab behind it", deliberate for `offload` rows), so asserting it about a row whose tab merely wasn't threaded is a false statement, not a missing one. Deliberately kept OUT of the F-20/F-16 pass to hold that diff to one seam; fix next. Sibling: `contract_drift`'s `Unattributed` is honest **only because** `ContractDriftBody` carries no `tab` — closing that is a shim/app wire change, also out of scope |
| F-30 | `detection/`, `themes/` and `palettes/` are read from disk by the test suite yet appear in **no `paths:` filter** in `tests.yml` | **DECIDED 2026-08-12 — invert to `paths-ignore:`; fix owed** | A commit touching only those is **untested *and* runless**. The new `gate-tests` job fails such a tag (correct — that tree really is untested), but the real fix is in `tests.yml`, which was out of that agent's scope. Related residual, not a finding: only the **head** of a push gets a Tests run, so tagging a mid-push commit fails the gate — also correct, also surprising |
| F-34 | **F-23's PROXIED twin:** after the same user flip, a *proxied* external call still gets `REFUSAL_EXTERNAL_BLOCKED` — *"this task has already used a local-capability tool"* — equally false | **FIXED 2026-08-12** — `REFUSAL_EXTERNAL_USER_LOCAL`, chosen in `gate` only; **2002/0/5, clippy clean** | Left untouched deliberately, because it **is** a decision and not a code change: the path is `LatchRegistry::gate` → `Latch::proxy_gate` → `Latch::refusal`, a **pure function over `Latch`** that the worker shares, and `REFUSAL_LOCAL_BLOCKED`'s doc calls these *"the two per-direction constants required by locked decision 2"* — so a third one amends that decision. It is also **pinned by an existing test** (`flip_local_reopens_local_tools_and_closes_the_external_side` asserts `Err(REFUSAL_EXTERNAL_BLOCKED)` after the flip), so it cannot be changed silently. **The cheap shape if you want it: post-process in `gate` only, which holds `entry.local_by_user_flip`, and NEVER in `Latch::refusal` — the worker has no user-flip concept.** Note F-23's fix is inert at the shipped default while this twin fires on the **proxied** path, which is live — so this is arguably the more visible half of the same defect |
| F-35 | With a **worker-only** detection override, `broken_local_rules` and `advisor_signal` return `None`, so a user whose own `rules.d/local/` files fail to compile is told **nothing** while the worker scans with them | **FIXED 2026-08-12** (decision 36) — `Scope::App` split into `Scope::AppWide` + `Scope::UnknownCaller`; both signals now gate on `Config::armed_anywhere(s)`. Retest boxes **11d/11e** | Not a false claim — a **suppressed signal**, so it is M-21's sibling rather than its duplicate, and it has the **same root cause: `Scope::App` is overloaded.** Both gate on `Config::from_settings(s, Scope::App).signature`. Closing it needs a decision M-21's did not settle: **should a card fire for a layer only the worker is running?** Decide with M-21's scope-vocabulary gap rather than in isolation. **Answered: yes** — one `rules.d/local/` on disk, one audience, and the worker screening with files that failed to compile is the state the card exists for. ⚠ **Two corrections found while fixing:** the second site is **`signature::advisor_signal`** (singular), not `updater::advisor_signals` (which takes no `Settings` and gates on nothing); and `updates_enabled` was never a *third* borrower — it asks the app-wide question and has been silently receiving the identity-less answer since N-1, so its own doc's *"`Scope::App` resolves to L1 ∧ L2"* was **false**. `updates_enabled` is deliberately **unmoved**; repointing it is **F-38** |
| F-33 | `010.0.0.0/8` produces **no SSRF candidate at all** — `is_plausible_host` rejects leading-zero octets while the WHATWG parser reads them as **octal** | ⚠ **PARTIAL 2026-08-12 — the measurement CONFIRMED AN SSRF BYPASS, and the finding named the wrong root cause. Leading-zero half fixed; see F-36 for the rest. DO NOT MARK CLOSED** | A pre-existing **extraction** gap of H-3's own family (screen and parser disagreeing on what a host *is*), found while proving M-18's exemption and **unrelated to it** — the M-18 predicate was asserted directly for that spelling so the exemption is not what hides it. Worth checking the whole leading-zero/octal class rather than the single spelling: if the fetcher's parser resolves `010.0.0.0` to `8.0.0.0`, then any address whose canonical form is private but whose written form has a leading zero is extracted by neither path. **Test it against the C-4 differential corpus, not by hand** |
| F-32 | The skipped-candidate signal from F-11's fix has an **operator** consumer (stderr) but **no user consumer** — a planted discovery entry that was skipped is a **security event** with nowhere to appear | **FIXED 2026-08-12** — `POST /activity/discovery_skipped` (`2013/0/5`, clippy clean) | The interesting case is precisely the one that now goes unreported: a planted entry was skipped **and the child reached the real app anyway**, so containment worked and nobody is told. The child holds a working, token-authenticated endpoint at that moment and could report it, but there is **no loopback route for a child to file an activity row**, and neither the plan nor decision 30 asked for one. Same family as the whole reporting-honesty workstream (F-23, F-24, M-5, M-21…M-24): containment right, reporting absent. Note `taint_beacon`'s safety argument is that it writes **nothing** to stdout/stderr and awaits nothing, so the signal deliberately lives in `proxy_base_for` and cannot simply be moved down into the shims |
| F-31 | The **OpenCode plugin gate ships default-off**, so every refusal it enforces — the pre-existing latch refusal *and* F-13's new contamination refusal — protects nobody at the shipped default | **DECIDED 2026-08-12 — DECLINED: stays default-off, decision 17 reaffirmed; docs owed** | Not a defect: `Feature::OpencodeNativeGate => false` (`injection.rs:269`) is **decision 17's deliberate choice**, and a test at `injection.rs:1428` pins it as the only default-off feature. Raised because it materially bounds what V32 claims for OpenCode tabs, and because two findings (F-13, and F-14's neighbourhood) were reasoned about as if the gate were live. **The decision owed: should the Phase H native gate ship default-ON?** Note the parallel with `native_web_visibility = sensor` for Claude tabs (locked decision 14) — the project has twice chosen report-only-or-off as the shipped posture for a *native* route it cannot intercept, which is coherent, but it means **the OpenCode native surface is ungated out of the box.** Decide with H-7 and V33's OS containment rather than in isolation |
| F-36 | ⚠ **CONFIRMED SSRF BYPASS — short-form dotted decimal with NO leading zero.** `127.1:8080/x`, `127.0.1:8080/x`, `10.1:8080/x`, `192.168.1:8080/x`, `172.16.1:8080/x`, `169.254.43518:8080/x` (cloud metadata), `//127.1/x` and the trailing-dot form `127.0.0.1.:8080/x` all produce **no candidate**, so the screen never runs, and the app's own `Url::parse` resolves every one of them to **loopback or private** | **FIXED 2026-08-12** (`2004/0/5`, clippy clean) — **its one residual is DECIDED: CLOSE IT, queued** | **F-33 named the wrong root cause.** Leading zeros are one of *four* families that escape; this one has no zero in it and `127.1` is the first entry on every SSRF bypass list. Measured against the real `candidates()`/`screen_urls()`, not read. Confined to `scan_bare_authorities` (schemeless + protocol-relative) — `scan_scheme_runs` never consults `is_plausible_host` and was never affected. **Decision taken 2026-08-12: close this family AND the hex/dword family (`0x7f.0.0.1`, `2130706433`, `017700000001`), EXEMPTING short forms whose FIRST octet is `0`** — `0.<n>` is the shape prose ratios take (`"mix them 0.5:1 by volume"` → `0.0.0.5`, inside `0.0.0.0/8`), and **no bypass spelling starts with `0` in a short form**, so the exemption costs no containment. **`0.0.0.0/8` stays denied for full 4-part spellings** — `0.0.0.0` routes to localhost on Linux. *Rejected:* dropping `0.0.0.0/8` from the schemeless deny set (cheaper, but trades a real target for prose comfort). ⚠ **Two method notes that cost a lap: the C-4 corpus CANNOT express this class** (its oracle parses the string as written, and `Url::parse("127.1:8080/x")` fails because `127.1` is not a scheme — the hole lives entirely in the *schemeless* scan), and **the fix must NOT normalize the host inside the extractor** — that is a second WHATWG IPv4 parser for the screen to disagree with, i.e. H-3's family again. Widen the predicate, emit verbatim, let `Url::parse` stay the single authority |
| F-39 | `/graph_run`'s `LatchScoping::Unknown(tab)` becomes `Attribution::Unrecognized(<body string>)` **with no length bound** | **FIXED 2026-08-12** — bounded at `LatchScoping::attribution()`, covering BOTH producers | A caller-supplied string from the request body reaches an activity row's attribution unbounded. F-32's own new route does bound it (`bounded_id`, applied **after** classification so truncation can never fold an invented id onto a configured one) — this is the sibling path that does not. Same family as F-16/F-20/F-29: what a row is allowed to *claim* about who produced it |
| F-37 | `CONTRACT_DRIFT_SEEN` is an **unbounded caller-keyed `HashSet`**, so any token-holder can evict the whole 400-row graph activity lane | **FIXED 2026-08-12** — key space is now STRUCTURAL (bound 6; a caller string cannot become a key) | ⚠ **F-32's implementation extracted the doubling counter into `outbound::Doubling`, which is exactly half this fix** — the other half is "key on something the caller does not control", which F-32's route already demonstrates. Deliberately left alone so a security route did not grow an unreviewed second change; it is cheap whenever it is scheduled | Found while designing decision 37's child→app route, in the route that turns out to already exist (`/activity/contract_drift`). Same family as the flood discipline decision 37 requires of the *new* route: a per-caller memo with no bound is an eviction primitive against a capped per-lane ring. Not part of F-32's own work — recorded separately so it is not closed by association |
| F-38 | Two sites are **documented as asking the app-wide question and behaviourally receiving the identity-less one**: `updater::updates_enabled` and `loopback::injection_status`'s `app` row. F-35's split made both spell `Scope::UnknownCaller`, which is now visibly the wrong-looking name at each | **FIXED 2026-08-12** — `armed_outside_the_worker`; zero behaviour change, asserted over all 36 states | Both were left exactly as they behave, because moving either is a **behaviour** change and decision 36's de-risking clause says the split must change nothing. `updates_enabled → AppWide` would stop the updater polling when detection is L2-off with one tab hardened (arguably right; **`→ armed_anywhere` is NOT an option — M-21 rejected it, and it would collapse `worker_only_detection` to a permanent `false` and orphan the Svelte branch gated on it**). `injection_status`'s `app` row → `AppWide` changes `GET /status` JSON and — the reason it is not a free win — **takes away the only observation point live-verify's N-1 box has**, which reads the elevation off that exact row (`decided_by:"scope"` beside `override_value:"inherit"`). Needs a user decision and its own retest box each |
| F-27 | `LOCAL_DATA_TOOLS` is **hand-mirrored** in `src/lib/settings/types.ts:1272` with no tripwire, and the mirror is stale | **FIXED — BOTH INSTANCES + TRIPWIRES** `2af9fe8`, `a4747f4` (mirrors) + `aadf49d` (tripwires) | Mirror corrected to Rust's 7 entries verbatim, and the brittleness underneath it fixed: preset identity is now **bidirectional set membership**, not array length, and the single writer + the reader both go through two exported helpers so they cannot diverge. Moved into `types.ts` because `.svelte` files have no test harness here, so the behaviour was untestable where it lived (11 new tests). **The owed `include_str!` tripwire must PARSE the array** (a per-name substring search would match a neighbouring comment) **and assert set equality in BOTH directions**, with a failure message saying the Settings window *writes* this list so a missing name is silently removed from the cloud exclusion. Pattern at `schema.rs:~4817`, `checks/mod.rs:~1010`. **⚠ THE SAME DEFECT HAS A SECOND INSTANCE, found 2026-08-11 by the M-3 fix: `src/SettingsApp.svelte` hand-mirrors the spawn-baked feature set in TWO places** — `restartShape()`'s `injection_spawn_baked` array and `injectionAppShape()` — and **both went stale the moment Spotlighting became spawn-baked.** Effect while it stands: flipping Spotlighting raises no *in-window* Settings hint. It does not hide the fix (the backend `ai-tab-restart-hint` still fires, `spawnStaleTabs` is the wider signal, and the "(needs a tab restart)" suffix renders from the backend's own `spawn_baked` report), but it is the same hand-mirror-with-no-tripwire shape. **Fix both mirrors under one tripwire, not as two one-offs.** HIGH-ish in effect, low in effort. Consequences while it stands: (a) `SettingsApp.svelte:994-1005` writes the stale 6-item exclusion whenever a backend's cloud flag is toggled, so **F-12's `LOCAL_DATA_TOOLS` half has NO PRODUCTION EFFECT** until this line changes — the Rust `default_for` is reached only by the router and tests; (b) `scopeMode()` (`SettingsApp.svelte:1007-1017`) matches on exact **length**, so a migrated install's 7-item list renders as *"custom"*, and clicking the *"web/docs only"* radio (`setScopeMode`, `:1018-1022`) writes the 6-item list back, silently dropping `run_check` from the exclusion. **No hole opens in any of these cases — `BackendGate` rule 4 still refuses at call time — but the UI lies.** Fix with an `include_str!` mirror tripwire; the pattern already exists at `schema.rs:4817` and `checks/mod.rs:1010`. Deliberately not added by the F-12 agent because it would have failed until the TS half landed, and it declined to hand back a red suite |
| F-13 | `/latch/state` publishes `contaminated`, but the OpenCode plugin's gate reads only `st.latch`, so a **contaminated-but-not-EXTERNAL** tab admits every local tool | **FIXED** `650c013` — ⚠ **BUT INERT AT THE SHIPPED DEFAULT**, confirmed deliberate by **F-31, 2026-08-12** | Refuses the **web** direction only, on `contaminated === true && latch === "open"`, via a **third** refusal constant `REFUSAL_NATIVE_WEB_TAINTED` (reusing a sibling's would state a cause that did not happen — F-23's defect). Closes the escalation `bash` → child `opencode` inherits `CIMP_TAB_ID` → new session id → latch reopens → web is back. `contaminated` deliberately does **not** join the type guard (that would be a total gate bypass); tests `=== "open"` so an unknown label still denies nothing; the 1500 ms abort, 2000 ms TTL and epoch drop-not-apply semantics untouched. Negative control run: `if (false)` makes the node-backed test fail. **⚠ THE BOUND, verified 2026-08-11: the refusal lives inside `if (CIMP_NATIVE_GATE_ENABLED …)` (`tabs/config.rs:1926`), and `Feature::OpencodeNativeGate => false` (`injection.rs:269`) — a test at `injection.rs:1428` asserts it is the ONLY default-off feature. So this fix, AND the whole OpenCode plugin gate including its pre-existing latch refusal, changes nothing for a user who has not enabled the Phase H gate.** That is decision 17's deliberate posture, not a defect in the fix — but the decision as filed over-claimed, and **whether that gate should ship default-on is an open posture question worth its own decision.** Also: F-13 adds no setting yet **changes what a FRESH tab launches with**, so an upgraded binary needs a tab restart before it is live. **reached live 2026-08-11** — an OpenCode tab sat `contaminated:true` with `latch:"open"` and rendered a contamination badge in that state. Signal with no consumer; decide with the contamination-clear path |
| F-14 | H-1's "most hardened combination" comment overstates what gate-ON + beacon-OFF invalidates | **FIXED** `650c013` | Replaced with two honest bounds: with the beacon off the invalidation is *structural* (it sits above the enable guard) but learns nothing new, and a **proxied** fetch triggers no invalidation there at all and rides the TTL alone. **The same claim was repeated verbatim in a test's doc comment** (`config.rs:4987`) — a copy the ledger never named, and the reason a comment-only finding still needs a grep rather than a single edit. LOW. Bounded live by recipe 19's second half, which verified the invalidation *does* sit above the beacon's enable guard |
| F-15 | `GraphIndex::mem_add_note` is still `pub` and takes a raw `&str`, so M-20's `NoteText` guards the *path*, not the *store* | **FIXED** `e06bd66` — **made unrepresentable, not detected** | The store now takes a `ScreenedNote` whose **only constructor runs the screen** and keeps the hits, and `secrets::screen` was demoted to module-private — so a verdict cannot be obtained without producing the value the store accepts, and the store accepts nothing else. Two properties now hold **by type-check** rather than by call-site discipline: *screenable* (`NoteText`, capped) and *screened*. No tripwire needed: all 14 call sites are compile errors until fixed. Test call sites go through a `#[cfg(test)]`-only helper that runs the **real** screen, so a fixture that would be held in production is held in the test too. **`NoteText`'s own doc overclaimed** — *"the note that gets stored is necessarily the note that was screened"* was a statement about **one caller**, not about the type; corrected. ✅ **The `mem_quarantined_notes` tripwire gap is CLOSED as locked decision 38 (2026-08-12) — `QuarantineReview`, a `PhantomData`-sealed capability taken BY VALUE (no `Clone`/`Copy`, so it cannot be stashed and reused), minted only by a private `for_memory_snapshot()` inside `graph/memory.rs` plus F-15's `#[cfg(test)]` sibling. TWO independent bounds, not one: the private field, AND `graph::memory` being a private module that deliberately does not re-export the type — so outside `crate::graph` it cannot even be NAMED, which forecloses the review's own hypothesised "an IPC command tomorrow". The snapshot assembly was MOVED into `memory.rs` to keep the constructor private rather than `pub(super)`; at 6,540 lines, `service.rs` would have widened it to the whole `graph::` tree including the model-facing `graph/mcp.rs`. `mem_quarantined_count` deliberately does NOT take the token — requiring it would mint one on the `context_notes` path (`graph/mcp.rs:1344`), handing the capability to the exact caller class it exists to exclude. Reasoning recorded on the fn itself.** It was a DIFFERENT mechanism from F-15: F-15 was *"the store accepts unvalidated input"* (fixed by constraining a write's argument type); that gap is *"the one method returning tainted rows has no restriction on **who may call it**"*, which needs a capability token whose constructor names the authorized consumer — a **design decision** (is it `memory_snapshot` today, an IPC command tomorrow, an export later?), not an implementation. **Residue: nothing prevents a new caller of `mem_quarantined_notes` from routing held note text into a read path, and the source scan sees datalog, not method calls.** |
| F-16 | Two of three `MemoryQuarantine` row producers write `root: ""` | **FIXED (Rust)** `37d44ac` — locked decision 33 | Both halves per the decision. Producer verdicts as actually found: `record_secret_screen_flag` **is** derivable but not from `idx` (root threaded through `run_tool`, 24 test sites); `unattributed_write` **is derivable CONTRARY TO THE FINDING** — of `gate`'s four call sites only `/graph_run` can reach a PERSISTENT-WRITE and that handler has `body.cwd`; `gate`'s scoped row was already correct. One resolver (`graph_root_key`) serves both the gate-time quarantine row and the post-dispatch row, so they **cannot name different projects**. **The sentinel rule is ENFORCED, not documented:** `Screen::requires_project_root()` is an **exhaustive match** — a new screen must answer — plus `rootless_forensic()` and a `debug_assert!` in `record_flag`. That is stronger than the planned test: it catches a **fourth** producer the planned test could not. Client half (sentinel doc mirror + the view rule) tracked separately. | filed LOW, but **reproduced live 2026-08-10**: of the two rows a dual-trip note writes, the one carrying `root:""` is the **secret-screen** row — in a root-filtered view the row recording that a *credential* was held is the one that vanishes |
| F-17 | Stale `cimp-<hash>.exe` from completed test runs wedges the next link with `LNK1104` | **OPEN — but it finally has a REPRO CONDITION, 2026-08-12** | LOW product / HIGH iteration cost; plausibly the same child-process leak as the two `audit::runner` kill-timing flakes — one fix may close both. ⚠ **The reason this row said "needs a repro, not a patch" is now half-answered:** on 2026-08-12 a full-suite run at `--test-threads=32` failed **exactly** `audit::runner::tests::cancel_kills_child` and `…::timeout_kills_child_and_reports_timed_out`, and **both passed in isolation seconds later** (`cargo test --bin cimp audit::runner::tests` → 29/0). **Reproduced TWICE that day, independently, on different trees — and five other full runs at the same thread count were clean**, so it is intermittent under contention rather than deterministic. So the flake is **contention-dependent, not code-dependent**, which is consistent with a child outliving its `kill()` under scheduler pressure rather than with a missing `kill_on_drop` — and `kill_on_drop(true)` really is set at every spawn site, so the ledger's original leak hypothesis stays unsupported. **Next step is to instrument the kill path under load, not to patch a spawn site.** Also a standing CI/agent hazard: a run at `--test-threads=32` can report 2 failures that a re-run does not reproduce, so **always re-run those two alone before calling a regression** |
| F-20 | Every proxied call's own `kind:"mcp"` row is `tab:"unattributed"`, and `graph` rows are `tab:"headless"`, while the `injection_flag` rows beside them name the tab | **FIXED** `37d44ac` | The recorders were already right; the **callers** passed `None`. Both handlers called `.into_scope()` immediately, **collapsing `Anonymous` and `Unknown` into one `None`** — attribution is now resolved *before* that collapse via a new `LatchScoping::attribution()` and threaded into `call_recorded`. **The parameter is a non-defaulted `Attribution`, so a caller can no longer express "nothing"** — a compile-level tripwire replacing the old `None`. **THE LOAD-BEARING CAUSE, confirmed: `graph/service.rs:1166` said *"No tab: this is the offload worker's in-process route"* while the same function's own doc says it backs the loopback `/graph_run` route. That one false comment is why every Claude and OpenCode graph AND memory call filed as `headless`.** `Attribution::Headless` on worker rows unchanged (now stated positively); `contract_drift` stays honestly `Unattributed` — its fix is a shim wire change belonging with F-6. Consumer verified, not assumed: `activity.ts::matchesTabFilter` already handles all four states, so **no frontend change was needed**. ⚠ **OWED: `a_graph_run_call_records_the_calling_tab_not_headless` was NOT written** — `activity::record_bg` has **no test seam** (it writes a process-global JSONL beside the exe), so asserting the row means mirroring `outbound::test_rows` for the whole activity store. **That missing seam is why this row's own invariant is only covered by live-verify — worth building.** **sharpened live 2026-08-11**: `refuse_headless`'s `ok:false` rows **do** name the tab while the served path's `ok:true` rows do not — same file, same `--tab`. The correct implementation is one function away (`graph/mcp.rs:744-747`); diff the two paths |
...
| F-22 | *(withdrawn)* live settings vs `settings.json` disagreement on `native_web_visibility` | **WITHDRAWN** | `43f4470`; issue [#52](https://github.com/Dyserna/cImp/issues/52) closed as invalid. My error — compared the resolved value against the global file alone, ignoring the sparse project overlay. **Read both config levels before any settings claim** |
| F-23 | After a **user** clicks "Switch to local", the refusal still says *"this task has already used a local-capability tool"* — a cause that did not happen | **FIXED (native path)** `218e100` — ⚠ **INERT AT THE SHIPPED DEFAULT** (behind Phase H's gate; F-31, 2026-08-12) · proxied twin **F-34 DECIDED 2026-08-12: fix** | Fixed the way F-13 established, so the locked *"refusals are fixed and content-free"* decision holds: a **distinct fixed constant**, not a dynamic message; it names no control and no escape hatch and refuses being re-asked. **Selecting it is a lookup on a fact cImp recorded about its own IPC** — `TabLatch::local_by_user_flip`, set in the same locked assignment as `entry.latch = Latch::Local`, beside the `latch_override` row with `origin:"ipc"`. Nothing from the caller, the model or the tool arguments reaches the message. **Containment byte-identical:** the condition is unchanged and only the thrown constant is chosen, pinned three ways including that the flag never joins the guard; a reply missing the field reads `false` and serves the old constant — **fail-open on the MESSAGE, never on the refusal.** The flag cannot outlive the latch it explains: all three writes to `TabLatch::latch` maintain it, and a latch **re-earned by a tool call** after a rotation correctly reports `false`. **Proved by EXECUTION through the real plugin**, not by greps. ⚠ **Inert at the shipped default (F-31's bound), and spawn-baked — a fresh OpenCode tab is required.** | the latch already writes a `latch_override` row with `origin:"ipc"`; the refusal just does not consult it. The quarantine notice next door DOES state a cause it checked — copy that pattern |
| F-24 | The Memory view's *Quarantined notes* card shows text + time + Promote/Discard only — **no screen, no rule, no reason** | **FIXED — BOTH HALVES** `0a874bc` (frontend) + `e06bd66` (backend) | The rule name is now the headline **above** the note text, so decision 22's ordering is right. **The backend half is real:** `MemNote` genuinely does not carry the reason (`graph/index/notes.rs:414-443`), so the card ships showing *"Reason not recorded — this build does not store which screen or rule held this note."* — the intended honest state for this RC, **not a bug to file**. Missing / `null` / blank all collapse to that one line, safe only because they collapse toward the honest end. Nothing is inferred from the note text (a test feeds a literal AWS key and asserts `null`). **DECIDED: publish the reason WITH the note, do NOT join to the activity row** — the `injection_flag` rows carry no `note_id` so a join has no key, and the activity store is a capped per-lane ring whose highest-volume lane is `MemoryQuarantine`, so **the oldest unreviewed notes — the ones most needing a reason — would blank first**. Set the field at the **store** boundary, not at the three callers, or F-15's "guard covers the path, not the store" gap reopens. Also fixed: the card's intro named only the latch cause, which is *why* a credential hold read as an injected-instruction hold. MEDIUM. Shows the **secret value** and withholds the **rule name** — the inversion of decision 22. The reason exists and reaches the model, which cannot act; not the human, who must. Take with M-23 + M-24 |
...
| F-26 | `graph/mcp.rs:609-613`'s one-byte repro does not reproduce — there are **two** discovery paths and the legacy one still resolves | **FIXED** `37d44ac` | Both comment sites now name **both** discovery stores, `select_discovery`'s three-step preference, and the trigger that is actually reachable by one write (`Transport`, **not** `NoInstance`). `ProxyMiss::Transport`'s doc — which read as a safety property it does not guarantee — and the operator message are corrected too. **Beyond comments: `select_discovery` now delegates to `select_with_legacy(entries, hint, legacy)`, making step 3 injectable so the property the wrong comment got wrong is finally ASSERTABLE** (production behaviour byte-identical). That is the general lesson: a comment-only finding whose claim was untestable is better closed by making it testable than by rewording it. raised live 2026-08-11. Cuts both ways: attack surface is *narrower* than documented (nothing about M-2 is weakened), but following the doc literally yields a **false PASS**. Fix the comment to name both files and say which `select_discovery` prefers |

## User decisions taken 2026-08-11 (session 9 fix phase) — settled, do not re-litigate

Recorded here rather than only in each row, because several of these were re-derived more than
once by independent agents. Each names the option chosen **and the argument against it**, so a
future reader can tell a considered trade-off from an oversight.

| Finding | Decision | The case against it, accepted knowingly |
|---|---|---|
| **F-9** | `SCAN_PASS_TIMEOUT = 2 s` **per pass**, not a shared total. Worst-case screen wall clock **1 s → 4 s** (typical ~210 ms) | A 4 s worst case on the fetch path. Forced: see F-9b — at N=1 the dependency's 1 Hz heartbeat gives a **zero** guaranteed floor, so the only alternative the heartbeat permits is not fixing F-9 |
| **F-11** | **Liveness-verified selection** (probe each candidate `GET /health` with its own token, skip non-answering, memoize per process, re-resolve on failure) **and `HttpStatus`/`Unparseable` STOP falling back** — today the app answered and *declined*, and we silently re-ran headless | Liveness proves something is **listening**, not that it is cImp: whoever can write the file can also bind a port and answer the probe. This raises the cost from one write to one write plus a listener; it does not remove the primitive. Rejected: refuse-on-ambiguity (breaks nested checkouts, the case the per-instance dir exists for), drop-deepest-root (reintroduces "project A's child talks to project B's app"), own-pid (a **no-op** — children have no own-pid file), HMAC (barred by locked decision 28 / H-6: the key would live inside the blast radius), bake-into-children (loses self-healing — a cImp restart would wedge a live tab headless for life; recorded as a deliberate **non-goal**) |
| **F-12** | The `checks_allow_remote_worker` opt-in is **per-project, global across backends** | A per-backend flag would let you trust a self-hosted LAN worker while denying a cloud vendor. Declined so this matches `graph.allow_remote_worker_access` exactly — two shapes for the same question is its own defect |
| **F-13** | Refuse the **web** direction only, on `contaminated && latch === "open"`, via a **third** refusal constant | Refusing local tools too would match how F-13 was filed, but it makes `FlipLocal` restore only half a surface and locks a **rotated, clean** conversation out of native `read`/`bash`. Contamination is sticky for the tab's life, so that would be close to a permanent withdrawal |
| **F-28** | Its own row, **fixed in the F-11 pass** | Folding it into F-11 would leave no row asserting the beacon can be disarmed, so nobody would re-test it |
| **M-1** | Thread **one task-scoped budget** through `run_on`, and **keep 40 calls / 4 MiB** — the documented number becomes the real number | On installs with ≥2 backends the per-task ceiling drops 160 → 40 calls / 16 → 4 MiB, so a tier escalation now inherits the fast attempt's spend and **some escalated answers will be less grounded than today's**. Choosing a truthful cap over answer quality on the ladder. Failure mode is soft (a tool-result refusal, not an abort). Rejected: raising the default to today's real 160/16 (ratifies a bound 4× what the spec reasoned about) |
| **M-5** | Derive the notice **per reason** from bytes that genuinely escaped screening — do **not** delete it, and do **not** reorder `cap_result` | Adds a `delivered_bytes` input and a `Gap` type to a helper whose current virtue is being one pure composition. Reordering was rejected outright: `agent.rs:1884-1892`'s rationale still holds, it re-breaks D-3, and it would discard forensic detection of hits in bytes 32 KB–256 KB. ⚠ **Correction to the finding: "false every time it fires" is only true of the two byte-prefix legs.** It is **true and load-bearing** for `classifier::MAX_WINDOWS` (bites at ~14 KB of dense tokens, inside what the model reads) and for every `incomplete` leg (yara-x timeout, failed inference, screening task that never ran) — so a blanket "suppress at the worker" would have deleted truthful signals |
| **M-16** | Pin `read` as an **ordered** object restating upstream's carve-out **verbatim** (`*`→allow, `*.env`→ask, `*.env.*`→ask, `*.env.example`→allow). Do **not** ship `OPENCODE_DISABLE_PROJECT_CONFIG`; do **not** widen past upstream's four patterns | Freezes out future upstream secret patterns until someone re-syncs. The env var stays **H-7's own work**: scoped default-on-for-hardened-tabs (as V19 §A.3 specced it) covers almost nobody, because the Phase H gate ships off — including the default install where M-16 was actually verified |
| **M-18** | **Narrow CIDR first** (exempt only canonical network addresses on schemeless runs: host bits zero + `/` + prefix `0..=31`, no port, no further segment), and **novelty-gate the denial rows as a SEPARATE, later change** (first denial per distinct target, 4 slots, then the doubling) | A scheme-guessing fetcher could now reach `http://127.0.0.0/8` on port 80 — a network address, portless, and not the loopback service decision 11 protects. H-3 is untouched, and `screen_urls` denies on **any** candidate so a dropped duplicate cannot flip a verdict. ⚠ **This row originally justified that with "all 21,120 corpus cases and all 30 C-4 rows are scheme-bearing, relative, or port-bearing" — WRONG IN DETAIL, corrected while implementing:** the C-4 row `10.0.0.1/admin` is none of those three (it is still not exempt, because its prefix segment is not numeric — right conclusion, different discriminator), and the real counts are **19,008** corpus cases and **29** C-4 rows. Kept visible because *"every case is X"* is exactly the kind of claim that reads as verified and is not. The novelty gate is deferred because it **breaks `ssrf_denial_rows_are_bounded_per_session_and_reset_on_rotation` and invalidates recipe 7's live doubling leg** — re-verification nobody has budgeted. "Exempt benign denials" was rejected as unimplementable: any proxy for "benign" decides *whether to audit* from model-authored text, which is the class C-4/H-3 close |
...
## User decisions taken 2026-08-12 (pre-RC decision pass) — settled, do not re-litigate

Every finding still open after the session 9 fix phase was either a **decision** rather than a
defect, or (F-17) an undiagnosed defect needing a repro. This pass settled all seven decisions in
one sitting so the RC has a fixed scope. Same convention as the table above: each names the option
chosen **and the argument against it**, so a future reader can tell a considered trade-off from an
oversight. Six of the seven produce code; F-31 produces documentation.

| Finding | Decision | The case against it, accepted knowingly |
|---|---|---|
| **F-34** | **Fix.** A **third** refusal constant, selected in `LatchRegistry::gate` — which holds `entry.local_by_user_flip` — and **never** inside `Latch::refusal`. Amends locked decision 2, whose doc calls these *"the two per-direction constants"*; correct that line. `flip_local_reopens_local_tools_and_closes_the_external_side` pins the old string: **split it, do not loosen it** | A third constant amends a locked decision, and the rule that keeps this correct — *never migrate the post-process down into `Latch::refusal`* — is a **convention, not a type**. It has to be, because `Latch::refusal` is a pure function over `Latch` that the offload **worker** also calls, and the worker has no user-flip concept to thread. Leave a comment or tripwire at the boundary. Taken because F-23's already-shipped fix is inert behind the default-off Phase H gate while this twin fires on the **proxied path, which is live** — so this is the more visible half of a defect already judged worth fixing |
| **F-31** | **Keep the Phase H OpenCode gate default-OFF, reaffirming locked decision 17.** The deliverable is documentation: record in the milestone **and** in the F-13 / F-23 / F-34 rows that those fixes are **inert at the shipped default** | Accepted knowingly: F-13, F-23 and F-34's OpenCode halves protect **nobody** out of the box until the user opts in or V33 lands. Rejected default-ON because the gate is **bypassable by design** — `OPENCODE_PURE=1`, spawning an ungated `opencode`, and user-typed `!shell`/PTY all walk around it — so shipping it on would imply containment it cannot deliver, on top of decision 16's argument that a control refusing native `bash` on a fresh install gets answered by switching V32 off wholesale. **The real problem F-31 found was not the posture but a comprehension hazard: the review itself kept reasoning about F-13 and F-14 as though the gate were live.** That is closed with documentation, not a flag flip |
| **F-35** | **Fix the `Scope::App` vocabulary now** — split it into distinct named questions and repoint all three borrowers: `updates_enabled`, `broken_local_rules`/`advisor_signals` (both asking *"does anything in this process use the shared bundle?"*), and the N-1 unattributed-caller path | **This edits `decide()`, the single source of truth for every V32 enforcement site, days before a 55-box retest** — the strongest argument for the narrower alternative (a local "effective in any scope" predicate inside the updater module), which was considered and rejected as leaving the root cause live. Mandatory de-risking, agreed as part of the decision: land it **before** the RC build, or the retest validates a tree that is about to change; ship it as **two commits**, a pure rename/split with zero behaviour change (the 1999-test suite is the guard) then the one intentional change; and **pin N-1 with a named test FIRST** — *"only `On` travels up, an L3 `Off` stays where the user put it"* — so the split cannot quietly widen or narrow it |
| **F-32** | **Add the child→app loopback route now**, so a child can file an activity row when it skips a planted discovery entry | Adds a **new token-authenticated write path into the activity store**, reviewed during RC week — the argument for deferring it a cycle, considered and rejected. The bar it must clear is recipe 21's forgery argument, already proven for `POST /latch/beacon`: a forged row leaves the registry untouched, and **the route answers identically on every path so the response is not the signal**. It cannot be pushed down into the shims instead: `taint_beacon`'s whole safety argument is that it writes nothing to stdout/stderr and awaits nothing, so the signal stays in `proxy_base_for` |
| **F-33** | **Measure before fixing.** Run the leading-zero family against the **C-4 parser-differential corpus** — above all **`0177.0.0.1` → `127.0.0.1`**, the inverse of the observed case. Fix by accepting leading-zero octets **and normalizing to the parser's own reading** only if an escape is confirmed | The row as filed observed no-candidate only on `010.0.0.0/8`, which resolves to a **public** address — a real screen/parser disagreement, but **not yet evidence of a hole**, which is why this is a measurement and not a patch. The measurement is the one that decides the RC's scope: **if a private-canonical form escapes both paths it is an SSRF bypass and blocks the release.** Rejected "accept anything numeric" as the fix shape — it widens the candidate set the screen pays for on every call, and both the screen's cost and its false-positive behaviour scale with candidate count |
| **F-30** | **Invert `tests.yml` from `paths:` to `paths-ignore:`** (`docs/**`, root `*.md`, issue templates). Leave `clippy.yml`'s allowlist alone | Costs CI minutes on commits an allowlist would have skipped. Taken because the allowlist **cannot be verified**: there is no reliable way to enumerate what the suite reads from disk, so it drifts every time a data directory is added — and it drifts **fail-open**, a forgotten entry meaning the tests silently do not run, which is how `detection/`, `themes/` and `palettes/` came to be untested *and* runless. Inverting flips drift to fail-safe and needs no tripwire, because nothing has to be kept in sync. `clippy.yml` keeps its allowlist because its entries genuinely are compile inputs, so that list **is** verifiable in the way this one was not |
| **`mem_quarantined_notes`** | **Capability token, mirroring F-15's "made unrepresentable, not detected".** The accessor takes a `QuarantineReview` whose **only** constructor is reachable from the `MemorySnapshot` path — **no `pub fn new()`** — plus a `#[cfg(test)]`-only constructor for the test sites | The ledger framed this as an open design question (*"is the authorized consumer `memory_snapshot` today, an IPC command tomorrow, an export later?"*). It does not have to be answered: **there is exactly ONE production caller**, `graph/service.rs:2059`, so today's consumer is nameable and tomorrow's can construct the token deliberately — which is precisely the code-review moment you want when someone starts routing held secret values somewhere new. Rejected module-privacy as too coarse to express an explicit opt-in, and value-wrapping as materially more invasive. ⚠ Worth stating plainly: three lines above that call site a **comment** already states the rule — *"the Memory UI is the only reader allowed to see them"* — making this the **sixth** instance of this codebase's signature defect, a comment standing in for a check |

**Not decided here, and deliberately so:** **F-17** remains the only unfixed *defect* and is still
undiagnosed — `kill_on_drop(true)` is set at every spawn site, so the ledger's leak hypothesis is
unsupported and it needs a **repro**, not a patch. **H-7 and F-4…F-8** stay V33 / OS-containment
territory, recorded as known-open and explicitly **not** RC blockers.
...
> **STATUS: FIXED — `7a38a05`.** User-decided shape: **demote both + strip signatures**, recorded as **locked decision 29** and the **Phase A amendment 2026-08-08 (c)** in the milestone — a defect in the locked decision itself, not code drift from it, so the taxonomy table was corrected rather than silently edited.
>
...
> **STATUS: FIXED — `3a0e31e`.** `URL_PREFIXES` → `URL_SCHEMES` (`["http:", "https:"]`): the slashes left the constant and are **parsed, not matched**. `scan_scheme_runs` matches the scheme case-insensitively, consumes the slash run (zero length allowed), and emits a candidate normalized to `//`. `\` is dropped from the terminator set only *inside a scheme-bearing run* — it stays a terminator for `scan_bare_authorities`, where it is a Windows path separator.
>
...
> **STATUS: FIXED — `c1722f7`.** Rules widened to `\s{1,8}`, plus `signature::normalize_for_scan` and a second unioned scan pass, plus three smoke-corpus controls so the updater’s gauntlet enforces it on every future bundle. Measured 20/50 hostile probes evading to **0/53**, with 15 benign controls still clean. Locked decision 7 amended.
...
> **STATUS: FIXED — `c1722f7`.** Channel repointed to a `raw.githubusercontent.com` branch ref (verified 200, no redirect), and the missing guard added: the pinned URL is now asserted against our own redirect policy, with release-asset paths rejected by name. Locked decision 27; decision 24 unblocked.
...
which no legitimate caller sends. `3109d2d` validated `/run`'s `tool` label at the
...
> **STATUS: FIXED — `db3e6db`.** **The type made the bug expressible**, so the type was the fix: `{armed} | null` conflated *a failed read* with *nothing to add*, and `SignatureHealth` is now `{armed} | 'unknown' | 'pending'` with **no `null` member** — the pass-through case must be typed as the word `'pending'`.
>
...
|---|---|---|---|
| M-1 | Worker budget/latch is per-**attempt**, not per-task — `run_on` is called up to 4× (fail-over, thinking retry, tier escalation), each with a fresh `Budget::default()`. The 4 MiB/40-call cap is really 8 MiB/80 on a default install, 16 MiB/160 with fail-over configured. A loop that resets its own cap is not stopped. | **FIXED** `d391bd8` — cap **KEPT at 40/4 MiB** per the decision, so the documented number is now the real one. New `TaskScope` (neither `Copy` nor `Clone`) minted **once** in `Service::run`, outside the ladder. **⚠ USER-VISIBLE REGRESSION, accepted and unsoftened: on installs with ≥2 backends the per-task ceiling drops 160 → 40 calls / 16 → 4 MiB, so an escalation after ~35 calls proceeds with ~5 left and SOME ESCALATED ANSWERS WILL BE LESS GROUNDED than before.** Default single-backend install with thinking off/auto is byte-identical. Failure mode stays **soft** (a tool-result refusal, never an abort). **A THIRD thing was resetting and the finding did not name it:** the `task_scope` id and the `TaskAudit` ledger were per-attempt, so one task's rows scattered across up to **four uncorrelatable** `task-xxxxxxxx` ids — now one id, one `Screen::Budget` row, one `LatchRefusal` row, one doubling ledger, one unscreened bit. **Two false comments deleted, both claiming *"the router's lifetime IS the task's"* — precisely what was untrue and what caused the scatter.** Forgetting to thread it is unrepresentable: required parameter, no `Default`, no fallback, and `AgentConfig::task_scope` **deleted** so an attempt cannot describe a different task than its loop; a source-scan tripwire guards the one case no type can (a future retry path calling `for_task()` again), because `Budget` must keep `Default` for `loopback::TabLatch`. **`mcp.rs`'s inert twin was FIXED, not recorded — *"inert today"* is how F-10 happened.** **`latch_flagged` still resets per attempt DELIBERATELY** (fresh conversation from clean inputs ⇒ no contaminated byte crosses the boundary); it was a **reporting** bug only, and that half is fixed. **This exposed loose language in locked decision 1 — "per task" — now amended: budget and reporting claims are per task, the latch is per ATTEMPT.** | `agent.rs:1502-1505`, `service.rs:1147-1252` |
| M-2 | The A-1 fix **inverted a fail-closed default**: a native name missing from `TABLE` used to classify EXTERNAL and be refused/latched; now it "neither latches nor is refused" and falls through to dispatch. Load-bearing in both directions, with no test and no tripwire. | **FIXED (invariant made mechanical, both directions; the A-1 fix kept)** `c337d6a` — see the M-2 note below; carries F-10's fix | `toolclass.rs` (`ClassRow::dispatchable`, `dispatchable`), `loopback.rs` (`LatchRoute::can_execute`, `LatchRegistry::gate`), `agent.rs::latch_gate`, `backend_gate.rs` |
| M-3 | Spotlighting is spawn-baked in practice (`fact_promotion_block` → `--append-system-prompt` / OpenCode instructions) but declared live: absent from `spawn_inject_sig`, no restart hint. Toggling it on mid-session leaves the running tab injecting **unenveloped pre-V32 memory into the system prompt**. Test pins the defect. | **FIXED** `650c013` — as a **BUILD ERROR**, not a source scan: `spawn_baked` is now a `const fn` and the call site goes through a new `Feature::baked_at_spawn()` that const-asserts, so a spawn-baked control missing from `spawn_inject_sig` fails compilation. `Consumer::reads(Spotlighting)` is now true for **both** consumers. The defect-pinning test moved `"spotlighting L3"` out of `stays` into `moves`. **Implementing it exposed three errors in its own plan, all real test failures:** a sibling test read `l2[0]` **positionally** and `Spotlighting` is declared before `NativeWeb`, so this fix pushed `native_web` off index 0; a derived L3 loop asserted a signature move for a feature **Claude does not read**, with a Claude-only fixture; and `Override::Off` over a **default-off** control is not a change at all, because `spawn_sig` carries resolved booleans | `config.rs:1082-1089`, `injection.rs:330-337,1736` |
| M-4 | A classifier that runs and fails is indistinguishable from Clean, and a failing window **discards windows that already scored over threshold**. `Scored` has no field to express it. The spec names exactly two exclusions; the code implements three. | **FIXED** `c1722f7` | `classifier.rs:391-399`, `mod.rs:404-409` |
| M-5 | At the worker, the "unscreened" notice is **false every time it fires**: `cap_result` truncates to 32 KB, unconditionally below both screening caps, so every byte the model sees was scanned. Trains the reader to discount a notice that is true at the proxy. | **FIXED** `1fa3be3` | Derived **per REASON** now, not per boundary: `Verdict.gaps` carries `examined_prefix: Option<usize>` and the notice takes `delivered`. **⚠ THE FINDING'S OWN CLAIM NEEDED CORRECTING, and that is why per-reason was the right shape:** *"false every time it fires"* holds **only** for the two byte-prefix legs. It is **TRUE and load-bearing** for `classifier::MAX_WINDOWS` and for **every `incomplete` leg** (yara-x timeout, failed inference, a screening task that never ran) — so the blanket reading, "suppress it at the worker", **would have deleted truthful signals.** `cap_result` was **NOT** reordered: its ordering rationale still holds, reordering re-breaks D-3, and it would discard forensic detection of hits in bytes 32 KB–256 KB. **Found and fixed in the same lines:** the classifier's two caps were ONE sentence, printing *"scored the first 64 KiB of 20 KiB"* when only the window cap fired — split into `truncated_input` + `window_capped`. And `cap_result` now routes through `result_cap_bytes`, so **the notice's number and the cut's number are one function** | `mod.rs:387-394`, `agent.rs:1910/1928` |
| M-6 | Audit findings enter model context with no envelope, no detection scan, and without contaminating the conversation — scanner-quoted text from `node_modules` is framed as authoritative project data, and `context_note` afterwards is *not* quarantined. `context_recall`, strictly tamer, *is* enveloped. | **FIXED (envelope + detection); contamination DECLINED with reasons** `227dcd0` — see the M-6 note below | `audit/mcp.rs:327-428,444-501`, `spotlight.rs:63-91,172-185`, `detection/mod.rs:555-584` |
| M-7 | Three ungated loopback routes reach local capability; `POST /context/post_edit` **executes the project's configured checks** for a caller-supplied `cwd`. None carries a `tab`; none appears in any route enumeration. | **FIXED (taint gate + enumeration)** `11e6231` — see the M-7 note below; the caller-supplied `cwd` half is **NOT** closed and is folded into H-7 | `loopback.rs:4095,4181,4690`, `toolclass.rs:234-266`, `config.rs:659,683,762,1872` |
| M-8 | `run_check` dispatches **above** the class gate on the headless path, so an EXTERNAL-latched tab that corrupts `.cimp-discovery` runs the project's build/test/lint while `ddg__*` stays live. | **FIXED (gate raised + widened to the class, keyed on the child's tab identity)** `343b6f2` — see the M-8 note below; the residual is the identity-less caller, which is F-5/H-8 | `graph/mcp.rs:520-524,632-706`, `offload/mcp.rs:400-414` |
| M-9 | The `#46` outcome split covers the manifest fetch but not the **artifact** fetch: any asset 404/timeout is recorded as a bundle *rejection* (red card, `unreachable_streak` reset). The deploy note's own publish order makes this the likely steady state. | **FIXED** `95be25d` — an artifact fetch failure is `Unavailable`, not a rejection | `updater/mod.rs:1032-1039,1059` |
| M-10 | A crash *during* rollback deletes the files the rollback already restored — the journal has two phases and the rollback is an unrepresented third state. Permanent, uncarded, and `warn!`s "the previous version was restored". | **FIXED** `95be25d` — `Phase::Restoring`; recovery no longer deletes what the rollback restored | `updater/mod.rs:1423-1434,1365-1370` |
| M-11 | `restore_archived` swallows per-file failures; the caller then reports "the previous version was restored" verbatim, and `healthy` cannot see missing files. Silent permanent coverage loss with a reassuring message. | **FIXED** `95be25d` — restore debt is durable and retried; verdict = health AND no debt | `updater/mod.rs:1401-1413` |
| M-12 | Crash recovery only runs when the updater is enabled *and* something is due. A user who turns detection off after a crash strands a short/empty `rules.d` permanently — "never degrade to no rules" fails closed on an unrelated switch. | **FIXED** `95be25d` — recovery no longer gated on enabled-and-due | `updater/mod.rs:1974-1999` |
| M-13 | An identifier collision between a shipped rule and a user rule freezes the update channel forever and blames the user's file — U-4's exact symptom, in the case the README tells users to expect. | **FIXED** `95be25d` — the user's rule is RENAMED (`custom_`), not dropped; **U-4 amendment WRITTEN 2026-08-11** (Phase C3 §"U-4 amendment 2026-08-11"; the stale "still fails and still rolls back" sentence marked SUPERSEDED in place). Verified while writing it: forgiveness now keys on the `local/` prefix **alone**, not "was failing before" (`updater/mod.rs:461-471`), only a *bundle* failure vetoes, an *introduced* `local/` failure is reported and deliberately NOT rolled back (`:472-513`), and a rename is a notice not a fault (`renamed` sits outside `Status::healthy`, `signature.rs:159-164`). Two copies of the retired claim also corrected (`MAINTENANCE.md`, and a live-verify **negative control that asserted the behaviour M-13 reversed**) | `updater/mod.rs:430-437` |
| M-14 | The updater's run lock is process-local; two instances sharing an exe directory race the swap and can destroy the old bundle with the journal pointing at an empty archive. | **FIXED** `95be25d` — `update.lock` via `create_new`, age-based staleness, never pid | `updater/mod.rs:490`, `store.rs:69` |
| M-15 | H-1's gate-cache fix narrows the race but does not close it: the epoch is bumped *before* the beacon POST and never after it resolves, so a query issued **during** the POST caches an `open` verdict for a full 2 s TTL. | **FIXED (the whole in-flight window is now a refusal, not just an un-cached verdict)** `98ab04d` — see the M-15 note below | `tabs/config.rs` (`opencode_plugin_source`: `CIMP_WEB_PENDING`, `cimpGateState`'s `settle`, the gate's `external`) |
| M-16 | `read` is deliberately left unpinned to preserve OpenCode's `*.env` → "ask" carve-out — but last-match-wins applies to the *project* config too. A repo shipping `{"permission":{"read":"allow"}}` resolves `read * → allow` and `.env` is read with no prompt. Verified live. | **FIXED** `650c013` — `read` is pinned as an **ordered** map restating upstream's four patterns **verbatim** (`*`→allow, `*.env`→ask, `*.env.*`→ask, `*.env.example`→allow), inside the hygiene branch so it vanishes with the other pins and cannot be resurrected by a denial. Not widened; `OPENCODE_DISABLE_PROJECT_CONFIG` deliberately **not** shipped (it stays H-7's work). **Insertion order is load-bearing under last-match-wins, and `serde_json`'s ordering here comes from a TRANSITIVE `preserve_order` feature — so the test asserts the SERIALIZED STRING, not the map.** A map-level test would pass while the emitted file was wrong. Adds no setting but **changes what a FRESH tab launches with**, so an upgraded binary needs a tab restart before it is live | `config.rs:2038-2043` |
| M-17 | `/mcp/call` and worker error paths carry the remote MCP server's `error.message` verbatim and up to 300 chars of raw response body — unscreened, unwrapped, unbudgeted — while both call sites' comments assert these are cImp-composed strings. | **FIXED** `37d44ac` — **the highest-value fix of this group: a hostile MCP server's error text was an unenveloped injection channel, and on the JSON-RPC path it had NO BOUND AT ALL.** Closed by splitting the bytes **by author at the type level**: `HostError { diagnostic, remote }` with no `Deref`, no `AsRef<str>`, and a single `remote()` accessor whose only caller is `wrap_remote_error` — so the remote half **cannot reach the model except through the envelope**. Bounded at capture (`MAX_REMOTE_ERROR_CHARS = 200`), a 4th standing-instruction preamble **registered in `ensure_closed`'s list**, and `compose` gained a `lead` param so header → notice → diagnostic → envelope keeps one definition. Connect paths deliberately still convert to `String`: a connect failure's reader is the Settings health row, i.e. **a human, not the model**. Flood bound verified rather than assumed — one row per flagged error, same `ScopeAudit` claim bits as a flagged success. Three false comments corrected. ⚠ **Accepted cost: the error paths now run detection (YARA + optional classifier) on the remote fragment — new work on a failure path** | `mcp_host.rs:1282-1314`, `loopback.rs:3804` |
| M-18 | The SSRF widening reads CIDR notation and doc placeholders as fetch targets: `"RFC1918 (10.0.0.0/8)"` in a *search query* refuses the whole call with a security error. Compounds — each benign denial raises the power-of-two threshold that suppresses a later real one. | **CIDR HALF FIXED** `aadf49d` — **novelty-gate half DEFERRED by decision** | Exempt only canonical network addresses on schemeless runs (Ipv4-parseable host, literal `/`, 1–2 digit prefix with no leading zero, `0..=31`, host bits zero, no port, no further segment); `10.0.0.1/8`, `10.0.0.1/32` and `10.0.0.0:80/8` all still deny. **H-3 is audited by its OWN generator** — the corpus gained a `/8` axis (15,840 → 19,008) and a new test drives the 3,168-case `/8` slice **non-vacuously** (>1,000 cases, >100 oracle hits); all 29 C-4 rows still refuse, checked row by row. The guard sits in `scan_bare_authorities` under `!relative`, and `strict` is structurally `false` for everything that function emits — a precondition held **by construction, not by convention**. Accepted cost written into the code: `http://127.0.0.0/8` reachable on port 80. **The second half stays deferred:** `claim_ssrf`/`is_power_of_two` untouched, `ssrf_denial_rows_are_bounded_per_session_and_reset_on_rotation` passes, recipe 7's doubling leg still valid. Raised in passing: **F-33** | `outbound.rs:386-415,660-684` |
| M-19 | Decision 21's "empty is not absent" reasoning is applied on the headless path but not the loopback path: a `PersistentWrite` with no resolvable tab identity is stored **unquarantined**, and `scoped_session` attributes it to another tab's session. | **FIXED (both defects; quarantine — not refusal — plus a write-only session resolver)** `c21178f` — see the M-19 note below | `loopback.rs` (`unattributed_write`, `LatchRegistry::gate`), `toolclass.rs` (`WriteTaint::Unattributed`, `write_notice`, `UNATTRIBUTED_WRITE_NOTICE`), `graph/mcp.rs` (`write_session`, the `context_note` arm) |
| M-20 | `context_note` text is unbounded and the secret screen only sees the first 256 KiB — 256 KiB of filler then an AWS key stores Clean. `secrets.rs:120-128` asserts the opposite ("it cannot reach either bound"). | **FIXED (bound made structural; the false claim corrected)** `c21178f` — see the M-20 note below | `graph/secrets.rs` (`NoteText`, `MAX_NOTE_BYTES`, the static assert, `screen`), `graph/mcp.rs` (the `context_note` arm's parse boundary) |
| M-21 | A worker-only detection override leaves the updater inert and the manual buttons lying: `updates_enabled` resolves at `Scope::App`, which deliberately excludes the worker row, so the UI says "detection is off" about a layer that is running. | **FIXED (backend)** `218e100` — **`src/` copy owed** | **Scope semantics deliberately UNCHANGED** — `updates_enabled` still resolves at `Scope::App`, because the defect was a **false statement about the world**, not a wrong resolution. Exactly one backend claim was false (`updates_allowed`); it now says detection is off app-wide and for every tab but **still ON for the offload worker**, which keeps screening with the bundle already on disk, and that the updater follows the app-wide answer so one worker override does not start it. The original sentence is retained on the branch where it is **true**. Reporting only: the buttons still refuse, asserted by a test named for the rule — **reporting honesty must not become a new capability.** **⚠ ON THE RECORDED "Tabs only, deliberately" RATIONALE: it is CORRECT as written, and the real cause is elsewhere — `Scope::App` is OVERLOADED.** Its own doc admits it *"stands in for two different questions"*, and `updates_enabled` borrows it for a **third** — *"does anything in this process use the shared bundle?"* — for which the worker row **is** in the population. A **scope-vocabulary gap**, not a resolution bug; worth fixing as vocabulary before something else borrows it for a fourth. Sibling with the same root cause, still open: **F-35**. Owed in `src/`: the row title, three button tooltips, one prose paragraph, and two additive `UpdaterStatus` fields | `updater/mod.rs:246-252`, `injection.rs:724-729` |
| M-22 | The override popover renders a click-time snapshot that never updates while open — a tab that becomes contaminated while the user reads it keeps saying "Not latched." | **FIXED** `0a874bc` — `taintMenu` now holds only the anchor `{x,y,tab}`; the row and the reduced-features list are `$derived` off the store the badge already reads. **No timer added and none needed** — `startLatchPolling` already runs app-wide from `App.svelte` on a 4 s tick, so there was no `appViewVisibility` gate to add. `TaintMenu.svelte` needed **zero** changes: it renders entirely from the `row` prop | `TabBar.svelte:68-102` |
| M-23 | Promote (unquarantines attacker-authored text into future sessions) is one unconfirmed click; Discard (which can only lose a note) is behind a modal. Polarity inverted vs. the latch UI, which gets it right. | **FIXED** `0a874bc` — Promote is now the danger-coloured two-step with the longer warning. **Discard deliberately KEEPS a confirmation:** the remedy is that Promote must cost *more*, and deleting a research conclusion is irreversible too — dropping that guard would have traded a reporting defect for a data-loss one. Note for future work: **there is no confirm-modal component in this repo** (the patterns are `TaintMenu`'s inline two-step and `ToolActivityView`'s armed button), so "reuse the modal" is not an available instruction | `CodeIntelligenceView.svelte:338-339` |
| M-24 | `Unscreened`, detector flags, `MemoryQuarantine` and `LatchOverride` collapse into one red chip; only denials are visually distinct. "We did not look at all of it" reads as "we blocked something" — the opposite. | **FIXED** `0a874bc` — **extended** the Events tab's existing `denied`/`flagged`/`ok`/`failed`/`signal` vocabulary to 12 statuses rather than inventing a second one, and converged **both** views on one implementation (new `StatusChip.svelte` + `rowStatus()` in the tested `.ts`). `denied` is now the only red. **Found while converging them:** `EventsView`'s `status()` read `!ok` as `denied` for `updater` rows too, but `updater` is the one source written outside `record_flag`, where `ok` is the **bundle** outcome — so **a rejected rules bundle was reporting as a blocked tool call.** Now matched on `source` before `ok` is consulted | `ToolActivityView.svelte:356-375` |
...
2. **`hook_*` are the first `TABLE` rows that are classified but not dispatchable.**
   ...
   exception.**
   **Closed by M-2** (`c337d6a`), with one correction to this paragraph: they
...
The scanner→structural refactors (`3cfefd5`, `4d76ea6`) did what they claimed — the
...
`41f7112` audited only `MILESTONE-V32-injection-hardening.md`, and audited it
against `9ea9e4b` while two further commits landed after. The spec itself came
...
`4060d7b`, not only the three claims above. **Seven** claims were false: those
...
was the only clean reset, which `f86c7f3`'s user-driven clear path replaced;
...
(including in `secrets.rs`, whose header `74e297e` edited three lines away).
...
also still unpinned, and the count is still 16 (verified 2026-08-09 at `4060d7b`,
...
### Progress against that disposition (updated 2026-08-08, `c1722f7` + `16279cc`)
...
**Later the same day** (H-1 `7a38a05`, H-2 `9db7f10`, H-3 `3a0e31e`, H-8
`11b4dba`, H-9), the blocking set is down to **H-7 and H-10**. H-9's entry above
...
without doing any work. H-4 (`c1722f7`) added the second pass without widening
...
was stashed and the failure reproduced at clean `HEAD` `f86c7f3` — 1842 passed
...
measured as non-hypothetical (28 `ok:false` rows in 162 live calls, `5b7ef25`).
...
**FIXED** `c337d6a` — `offload/backend_gate.rs`.
