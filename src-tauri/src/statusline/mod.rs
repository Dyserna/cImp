...
//!     GUI.
//!
//! **V35 Phase K: the Claude-shaped half of this file moved out.** Reading a
//! `context_window` block or a `rate_limits` object is knowledge of Claude
//! Code's payload, so [`crate::harness::claude::statusline`] owns it now —
//! including the side channel below. What stays here is what is cImp's: the
//! `--statusline` entry point, the shell-safe command strings, and the coloured
//! bar itself.
//!
...
use crate::harness::claude::statusline::{extract_push, extract_push_meta, Input};
...
// ---- Claude-shaped payload parsing lives in `harness::claude::statusline` --
