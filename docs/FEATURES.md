...
- Reserved **Code Audit** dashboard tab (opt-in `code_audit.enabled`, off by default): user-triggered scan of the project root, per-tool progress, one aggregated findings table, select + copy-to-agent — no build step, scans manifests/lockfiles on a fresh clone
...
- All three emit SARIF, normalized through the V22 `sarif` parser into one merged findings table (severity · tool · rule · `file:line` · message) with severity-threshold / per-tool / text filters and a Graph View ⌖ jump
