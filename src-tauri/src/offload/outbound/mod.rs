...
// ── The address and URL predicates ─────────────────────────────────────────
//
// V42 R19 (#126): the IP-range policy and the URL extractor moved VERBATIM to
// [`urlscan`] — eighteen pure predicates that read no `Policy`, no `Budget`
// and no `Flag`, and the tests that pin them went with them. Everything below
// this line is the part that DOES: the carve-outs, the screen, the budgets,
// the canary and the `injection_flag` row.
//
// The glob re-export is the point of the split being pure motion: every
// `crate::offload::outbound::…` path a consumer already writes still resolves,
// and the screen below reads `candidates`/`Candidate`/`is_denied_ip` exactly as
// it did when they were declared in this file.
mod urlscan;

pub use self::urlscan::*;
