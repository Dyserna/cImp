...
- [ ] **Next:** ast-grep `struct_search` (Stage 4); loopback `POST /graph/query` warm path + native-tool router entry for the offload worker (rest of Phase C); more langs/docs (E — `tags.scm` for TS/JS/Py + Markdown doc-chunking); settings UI + `graph_pause_watch`/`graph_rebuild_embeddings`/`graph_test_query` (F); semantic/embeddings (G); monitor tab (I).
